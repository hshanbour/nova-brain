...
$arguments='"'+$script+'"'; $action=New-ScheduledTaskAction -Execute $node -Argument $arguments -WorkingDirectory $root
...
$environment='NOVA_LOCAL_WORKER_URL='+$PreviewUrl.TrimEnd('/')
$wrapped='cmd.exe'; $wrappedArgs='/d /s /c "set "'+$environment+'" && "'+$node+'" '+$arguments+'"'; $action=New-ScheduledTaskAction -Execute $wrapped -Argument $wrappedArgs -WorkingDirectory $root
Register-ScheduledTask -TaskName $name -Action $action -Trigger $trigger -Settings $settings -Description 'Bounded Nova Self-Development worker; credentials load from Windows Credential Manager.' -Force | Out-Null
