...
$arguments='"'+$script+'"'; $taskAction=New-ScheduledTaskAction -Execute $node -Argument $arguments -WorkingDirectory $root
...
$wrapped='cmd.exe'; $wrappedArgs='/d /s /c "set "'+$environment+'" && "'+$node+'" '+$arguments+'"'; $taskAction=New-ScheduledTaskAction -Execute $wrapped -Argument $wrappedArgs -WorkingDirectory $root
