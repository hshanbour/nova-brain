...
const {previewUrl:argumentUrl,gitExecutable:gitArgument,codexExecutable,credentialHelper,repositoryRoot:repositoryArgument,runtimeVersion,verifyRuntime,probeOnly}=parsePersistentWorkerArguments();
...
if(verifyRuntime){
  if(!runtimeVersion||!repositoryArgument||!gitArgument||!codexExecutable)throw Object.assign(new Error("Runtime verification requires explicit immutable bindings."),{code:"worker_runtime_binding_required"});
  process.stdout.write(JSON.stringify({ok:true,runtimeVersion,runtimeScriptPath,repositoryRoot,gitExecutable:gitArgument,codexExecutable}));
  process.exit(0);
}
...
await runPersistentWorkerService({baseUrl:base,repositoryRoot,intervalMs:interval,version:runtimeVersion||process.env.NOVA_WORKER_VERSION||"local",startupMetadata:{runtimeVersion:runtimeVersion||null,runtimeScriptPath,previewUrl:base,repositoryProof:{proven:true,contextVersion:repositoryProof.version,root:repositoryProof.root,repository:repositoryProof.repository,branch:repositoryProof.actualBranch,head:repositoryProof.actualHead,gitLayout:repositoryProof.gitLayout},startupStage:"repository_proof",cwd:repositoryProof.root,nodeExecutable:process.execPath,gitExecutable,codexExecutable,credentialHelperExecutable:credentialStore.helper.executable,credentialHelperKind:credentialStore.helper.kind,credentialHelperSelectionSource:credentialStore.helper.selectionSource,credentialHelperNativeExists:credentialStore.helper.nativeExists,credentialHelperFallbackUsed:credentialStore.helper.fallbackUsed},credentialLoader:options=>loadLocalWorkerCredentials({...options,store:credentialStore}),probeOnly,maxIterations:probeOnly?3:Infinity,shouldStop:()=>stopping});
