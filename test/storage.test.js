...
test("Postgres atomic Worker claim supports exact waiting-for-worker handoff",async()=>{const source=await import("node:fs/promises").then(({readFile})=>readFile(new URL("../src/storage/postgres-storage.js",import.meta.url),"utf8"));assert.match(source,/jsonb_build_object\('claimKey',\$6::text\)/);assert.match(source,/FOR UPDATE SKIP LOCKED LIMIT 1/);assert.match(source,/status IN \('queued','retrying','waiting_for_worker'\)/);});
test("Postgres task updates fence cancelled terminal records",async()=>{const source=await import("node:fs/promises").then(({readFile})=>readFile(new URL("../src/storage/postgres-storage.js",import.meta.url),"utf8"));assert.match(source,/status<>'cancelled' AND \(\$\$\{versionParameter\}/);});
test("Postgres autonomy step results preserve canonical diagnostics as JSONB on reload",async()=>{const source=await import("node:fs/promises").then(({readFile})=>readFile(new URL("../src/storage/postgres-storage.js",import.meta.url),"utf8"));assert.match(source,/key === "result" \? JSON\.stringify\(value\) : value/);assert.match(source,/key === "result" \? "::jsonb" : ""/);assert.match(source,/SELECT \* FROM nova_autonomy_steps WHERE task_id=\$1/);});
...
test("bounded maxSteps updates persist monotonically in memory and Postgres",async()=>{const storage=await seededStorage(),task=await storage.createAutonomyTask({id:"budget-task",ownerId:OWNER_ID,projectId:"nova-brain",title:"Budget",objective:"Bounded recovery",branch:"feat/nova-brain-mvp-foundation",startingCommit:"a".repeat(40),maxSteps:16}),updated=await storage.updateAutonomyTask(task.id,OWNER_ID,{maxSteps:44},task.stateVersion);assert.equal(updated.maxSteps,44);assert.equal((await storage.getAutonomyTask(task.id,OWNER_ID)).maxSteps,44);const monotonic=await storage.updateAutonomyTask(task.id,OWNER_ID,{maxSteps:20},updated.stateVersion);assert.equal(monotonic.maxSteps,44);for(const value of [0,101,1.5,"44"])await assert.rejects(()=>storage.updateAutonomyTask(task.id,OWNER_ID,{maxSteps:value},monotonic.stateVersion),error=>error.code==="invalid_max_steps");const source=await import("node:fs/promises").then(({readFile})=>readFile(new URL("../src/storage/postgres-storage.js",import.meta.url),"utf8"));assert.match(source,/maxSteps:\s*"max_steps"/);assert.match(source,/GREATEST\(\$\{fields\[key\]\},\$\$\{i \+ 3\}::int\)/);assert.match(source,/patch\.maxSteps > 100/);});
