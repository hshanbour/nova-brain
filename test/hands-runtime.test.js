...
test("task-bound patch accepts only an exact same-task prior-output dirty subset",async()=>{const f=await fixture();try{const {stdout}=await run("git",["rev-parse","HEAD"],{cwd:f.root}),sha=stdout.trim(),runId="selfdev_dirty_recovery",priorOutput="task-owned current\n",content="export const alpha = 2;\n",betaExpected="alpha docs\n";await writeFile(join(f.root,"alpha.js"),priorOutput,"utf8");const files=[{path:"alpha.js",operation:"replace",expectedContent:"export const alpha = 1;\n",content},{path:"beta.md",operation:"replace",expectedContent:betaExpected,content:"repaired docs\n"}],taskOwnedDirtyLineage={version:1,taskId:runId,repository:"hshanbour/nova-brain",branch:BRANCH,currentCommit:sha,sourcePlanStepId:"26:plan_implementation",sourceApplyStepId:"27:apply_patch",entries:[{path:"alpha.js",contentHash:canonicalContentHash(priorOutput)}]},planProvenance={version:"2",generationId:"rebound-generation",taskId:runId,currentCommit:sha,mutationPreconditions:files.map(file=>({path:file.path,operation:file.operation,expectedContentHash:canonicalContentHash(file.expectedContent)})),taskOwnedDirtyLineage};const result=await f.registry.execute("repo_apply_patch",{branch:BRANCH,currentCommit:sha,planProvenance,files},{runId});assert.deepEqual(result.files,["alpha.js","beta.md"]);assert.equal(await readFile(join(f.root,"alpha.js"),"utf8"),content);assert.equal(await readFile(join(f.root,"beta.md"),"utf8"),"repaired docs\n");}finally{await f.close();}});

test("focused subset repair preserves and refreshes the complete proven dirty lineage",async()=>{const f=await fixture();try{const extra=Array.from({length:6},(_,index)=>`owned-${index}.js`),paths=["alpha.js","beta.md",...extra];for(const [index,path]of extra.entries())await writeFile(join(f.root,path),`export const base${index} = true;\n`,"utf8");await run("git",["add","."],{cwd:f.root});await run("git",["commit","-m","eight tracked files"],{cwd:f.root});const {stdout}=await run("git",["rev-parse","HEAD"],{cwd:f.root}),sha=stdout.trim(),runId="selfdev_partial_repair",prior=Object.fromEntries(paths.map((path,index)=>[path,path.endsWith(".js")?`export const novaOwned${index} = true;\n`:`nova owned ${index}\n`]));for(const path of paths)await writeFile(join(f.root,path),prior[path],"utf8");const targets=paths.slice(0,3),replacement=(path,index)=>path.endsWith(".js")?`export const focusedRepair${index} = true;\n`:`focused repair ${index}\n`,files=targets.map((path,index)=>({path,operation:"replace",expectedContent:`base ${index}\n`,content:replacement(path,index)})),lineage={version:1,taskId:runId,repository:"hshanbour/nova-brain",branch:BRANCH,currentCommit:sha,sourcePlanStepId:"41:plan_repair",sourceApplyStepId:"42:apply_patch",activePlanStepId:"49:plan_repair",entries:paths.map(path=>({path,contentHash:canonicalContentHash(prior[path])}))},planProvenance={version:"2",generationId:"focused",taskId:runId,currentCommit:sha,mutationPreconditions:files.map(file=>({path:file.path,operation:file.operation,expectedContentHash:canonicalContentHash(file.expectedContent)})),taskOwnedDirtyLineage:lineage};const result=await f.registry.execute("repo_apply_patch",{branch:BRANCH,currentCommit:sha,planProvenance,files},{runId,stepId:"50:apply_patch"});assert.deepEqual(result.files,targets);for(const [index,path]of paths.entries())assert.equal(await readFile(join(f.root,path),"utf8"),index<3?replacement(path,index):prior[path]);assert.deepEqual(result.taskOwnedDirtyLineage.entries.map(entry=>entry.path),[...paths].sort());for(const entry of result.taskOwnedDirtyLineage.entries)assert.equal(entry.contentHash,canonicalContentHash(await readFile(join(f.root,entry.path),"utf8")));assert.equal(result.taskOwnedDirtyLineage.sourcePlanStepId,"49:plan_repair");assert.equal(result.taskOwnedDirtyLineage.sourceApplyStepId,"50:apply_patch");const secondPath=targets[0],secondFile={path:secondPath,operation:"replace",expectedContent:"stale base\n",content:"export const secondFocusedRepair = true;\n"},secondProvenance={version:"2",generationId:"second",taskId:runId,currentCommit:sha,mutationPreconditions:[{path:secondPath,operation:"replace",expectedContentHash:canonicalContentHash(secondFile.expectedContent)}],taskOwnedDirtyLineage:{...result.taskOwnedDirtyLineage,activePlanStepId:"57:plan_repair"}},second=await f.registry.execute("repo_apply_patch",{branch:BRANCH,currentCommit:sha,planProvenance:secondProvenance,files:[secondFile]},{runId,stepId:"58:apply_patch"});assert.equal(await readFile(join(f.root,secondPath),"utf8"),secondFile.content);assert.equal(second.taskOwnedDirtyLineage.entries.length,8);}finally{await f.close();}});

test("focused subset repair rejects complete-lineage drift unrelated dirt and out-of-scope paths before mutation",async()=>{for(const variant of ["drift","unrelated","outside"]){const f=await fixture();try{const {stdout}=await run("git",["rev-parse","HEAD"],{cwd:f.root}),sha=stdout.trim(),runId=`selfdev_partial_${variant}`,prior={"alpha.js":"export const ownedAlpha = true;\n","beta.md":"owned beta\n"};for(const path of Object.keys(prior))await writeFile(join(f.root,path),prior[path],"utf8");const target=variant==="outside"?"gamma.js":"alpha.js";if(variant==="drift")await writeFile(join(f.root,"beta.md"),"one byte drift!\n","utf8");if(variant==="unrelated")await writeFile(join(f.root,"unrelated.txt"),"unrelated\n","utf8");const file={path:target,operation:variant==="outside"?"create":"replace",expectedContent:"base\n",content:"export const safeReplacement = true;\n"},lineage={version:1,taskId:runId,repository:"hshanbour/nova-brain",branch:BRANCH,currentCommit:sha,sourcePlanStepId:"41:plan_repair",sourceApplyStepId:"42:apply_patch",activePlanStepId:"49:plan_repair",entries:Object.entries(prior).map(([path,content])=>({path,contentHash:canonicalContentHash(content)}))},planProvenance={version:"2",generationId:"focused",taskId:runId,currentCommit:sha,mutationPreconditions:[{path:target,operation:file.operation,expectedContentHash:canonicalContentHash(file.expectedContent)}],taskOwnedDirtyLineage:lineage},before=await readFile(join(f.root,"alpha.js"),"utf8");await assert.rejects(()=>f.registry.execute("repo_apply_patch",{branch:BRANCH,currentCommit:sha,planProvenance,files:[file]},{runId,stepId:"50:apply_patch"}),error=>error.code==="working_tree_dirty");assert.equal(await readFile(join(f.root,"alpha.js"),"utf8"),before);}finally{await f.close();}}});

test("task-owned dirty lineage rejects byte drift and another task before mutation",async()=>{for(const mutation of ["hash","task"]){const f=await fixture();try{const {stdout}=await run("git",["rev-parse","HEAD"],{cwd:f.root}),sha=stdout.trim(),runId="selfdev_dirty_recovery",priorOutput="task-owned current\n";await writeFile(join(f.root,"alpha.js"),priorOutput,"utf8");const file={path:"alpha.js",operation:"replace",expectedContent:"export const alpha = 1;\n",content:"export const alpha = 2;\n"},planProvenance={version:"2",generationId:"repair",taskId:runId,currentCommit:sha,mutationPreconditions:[{path:file.path,operation:"replace",expectedContentHash:canonicalContentHash(file.expectedContent)}],taskOwnedDirtyLineage:{version:1,taskId:mutation==="task"?"another-task":runId,repository:"hshanbour/nova-brain",branch:BRANCH,currentCommit:sha,sourcePlanStepId:"26:plan_implementation",sourceApplyStepId:"27:apply_patch",entries:[{path:file.path,contentHash:mutation==="hash"?canonicalContentHash("different\n"):canonicalContentHash(priorOutput)}]}};await assert.rejects(()=>f.registry.execute("repo_apply_patch",{branch:BRANCH,currentCommit:sha,planProvenance,files:[file]},{runId}),error=>error.code==="working_tree_dirty"&&error.safeDiagnostics.taskOwnedDirtyProven===false);assert.equal(await readFile(join(f.root,"alpha.js"),"utf8"),priorOutput);}finally{await f.close();}}});
