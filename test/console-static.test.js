...
test("Start Voice publishes microphone connecting state while provider readiness runs in parallel",async()=>{const script=await readFile(new URL("../assets/console.js",import.meta.url),"utf8");assert.match(script,/const readinessPromise=voiceClient\.readiness\(\);const startPromise=voiceV2\.start\(\);const \[readiness,started\]=await Promise\.all/);assert.match(script,/if\(!readiness\.available\)\{if\(started\)voiceV2\.end\(\)/);});
...
test("anonymous voice familiarity is explicit, session-scoped, revocable, and never presented as authentication",async()=>{const [html,script,client]=await Promise.all([readFile(new URL("../index.html",import.meta.url),"utf8"),readFile(new URL("../assets/speaker-familiarity.js",import.meta.url),"utf8"),readFile(new URL("../assets/voice-v2-client.js",import.meta.url),"utf8")]);assert.match(html,/id="speakerFamiliarityConsent"/);assert.match(html,/does not verify my identity, grant permissions, or retain raw audio/);assert.match(script,/event\.isTrusted/);assert.match(script,/credentials:"same-origin"/);assert.match(script,/Forget this anonymous voice/);assert.match(script,/let token=null/);assert.doesNotMatch(script,/localStorage|sessionStorage/);assert.match(client,/familiarityConsent/);});
...
test("local static handler serves all premium Voice V2 browser modules", async () => {
  ...
  for (const path of ["/assets/voice-v2-client.js", "/assets/voice-capture.js", "/assets/voice-v2.js", "/assets/speaker-familiarity.js"]) { const res = response(); assert.equal(await serve({ method: "GET", url: path }, res), true); assert.equal(res.statusCode, 200); assert.equal(res.headers.get("content-type"), "text/javascript; charset=utf-8"); }
});
