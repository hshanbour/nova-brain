...
test("Console exposes premium Voice V2 while retaining local Test Voice diagnostics",async()=>{const [html,script]=await Promise.all([readFile(new URL("../index.html",import.meta.url),"utf8"),readFile(new URL("../assets/console.js",import.meta.url),"utf8")]);for(const id of ["testVoiceButton","voiceDiagnostic","voiceModeButton","endVoiceButton","voiceModeStatus"])assert.match(html,new RegExp(`id="${id}"`));assert.match(script,/createMediaVoiceCapture/);assert.match(script,/createVoiceV2/);assert.match(script,/sendMessage\(text,\{autoSpeakResponse:false,throwOnError:true\}\)/);assert.match(script,/voiceV2\.interrupt\(\)/);assert.match(script,/input\.value="";resizeInput\(\);return sendMessage/);});
