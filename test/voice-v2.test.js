...
test("an unknown speaker pause phrase cannot use the verified-owner fast path",async()=>{const owner={match_status:"confirmed",speaker_profile_id:"owner-profile",speaker_label:"owner"};const unknown={match_status:"unknown",speaker_profile_id:null,speaker_label:"unknown"};const flow=setup({transcript:["long answer request","استني"],speaker:[owner,unknown],relevance:[{category:"addressed_to_nova",accepted_as_turn:true},{category:"likely_background_speech",accepted_as_turn:false}]});await flow.mode.start();await flow.audio();flow.started();flow.barge();assert.equal(await flow.audio(),false);assert.equal(flow.counts().sends,1);assert.equal(flow.mode.getState(),"speaking");});
