...
test("streamed playback stays pauseable and checkpointed while waiting between chunks",async()=>{const players=[];let releaseSecond;const secondReady=new Promise(resolve=>{releaseSecond=resolve;});class AudioMock{constructor(){this.listeners=new Map();this.currentTime=1.5;players.push(this);}addEventListener(name,callback){this.listeners.set(name,callback);}play(){queueMicrotask(()=>this.listeners.get("playing")?.());return Promise.resolve();}pause(){}removeAttribute(){}load(){}end(){this.listeners.get("ended")?.();}}const playback=createAudioPlayback({Audio:AudioMock,URL:{createObjectURL:()=>`blob:${players.length}`,revokeObjectURL(){}}});playback.play({async *[Symbol.asyncIterator](){yield new Blob(["one"]);await secondReady;yield new Blob(["two"]);}},{});await new Promise(resolve=>setImmediate(resolve));players[0].end();await new Promise(resolve=>setImmediate(resolve));assert.equal(playback.pause(),true);const gap=playback.checkpoint();assert.equal(gap.paused,true);assert.equal(gap.lastFullyPlayedChunk,0);releaseSecond();await new Promise(resolve=>setImmediate(resolve));assert.equal(players.length,1);assert.equal(playback.resume(),true);await new Promise(resolve=>setImmediate(resolve));assert.equal(players.length,2);});

test("soft barge duck persists across streamed chunks and restores without pausing",async()=>{const players=[];class AudioMock{constructor(){this.listeners=new Map();this.volume=1;players.push(this);}addEventListener(name,callback){this.listeners.set(name,callback);}play(){queueMicrotask(()=>this.listeners.get("playing")?.());return Promise.resolve();}pause(){}removeAttribute(){}load(){}end(){this.listeners.get("ended")?.();}}const playback=createAudioPlayback({Audio:AudioMock,URL:{createObjectURL:()=>`blob:${players.length}`,revokeObjectURL(){}}});playback.play({async *[Symbol.asyncIterator](){yield new Blob(["one"]);yield new Blob(["two"]);}},{});await new Promise(resolve=>setImmediate(resolve));assert.equal(playback.duck(.18),true);assert.equal(players[0].volume,.18);players[0].end();await new Promise(resolve=>setImmediate(resolve));assert.equal(players[1].volume,.18);assert.equal(playback.unduck(),true);assert.equal(players[1].volume,1);});
