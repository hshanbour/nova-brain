...
test("Self-Development expiry recovery route is exact-task and scoped-worker protected",async()=>{let input;const app=createApi({agent:{tools:{list(){return[];}},async run(){throw new Error("unused");}},config:{allowedOrigins:[],maxBodyBytes:64*1024,localWorkerToken:"local-only"},storage:{provider:"memory",durable:false},initialize:async()=>{},ownerId:"owner",selfDevelopmentExpiryRecovery:{async recover(value){input=value;return{task:{status:"queued"}};}},logger:{info(){},error(){}}}),body=JSON.stringify({runtimeMinutes:15,expectedVersion:51}),denied=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact/recover-post-attestation-expiry",headers:{"content-type":"application/json"},body}),denied);assert.equal(denied.statusCode,401);const allowed=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact/recover-post-attestation-expiry",headers:{authorization:"Bearer local-only","content-type":"application/json"},body}),allowed);assert.equal(allowed.statusCode,200);assert.equal(input.taskId,"exact");});
...
test("self-development review recovery requires scoped local Worker authorization",async()=>{let calls=0;const app=createApi({agent:{tools:{list(){return[];}},async run(){throw new Error("unused");}},config:{allowedOrigins:[],maxBodyBytes:64*1024,localWorkerToken:"local-only"},storage:{provider:"memory",durable:false},initialize:async()=>{},ownerId:"owner",selfDevelopment:{async recoverReview(){calls+=1;return{task:{status:"waiting_for_worker"}};}},logger:{info(){},error(){}}}),body=JSON.stringify({approvalId:"old",expectedCommit:"a".repeat(40)}),headers={"content-type":"application/json"};const denied=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/self-1/recover-review",headers,body}),denied);assert.equal(denied.statusCode,401);const allowed=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/self-1/recover-review",headers:{...headers,authorization:"Bearer local-only"},body}),allowed);assert.equal(allowed.statusCode,200);assert.equal(calls,1);});
test("discovery-only replan is exact-task and scoped-worker protected",async()=>{let taskId,input;const app=createApi({agent:{tools:{list(){return[];}},async run(){throw new Error("unused");}},config:{allowedOrigins:[],maxBodyBytes:64*1024,localWorkerToken:"local-only"},storage:{provider:"memory",durable:false},initialize:async()=>{},ownerId:"owner",selfDevelopment:{async replanDiscoveryOnly(id,value){taskId=id;input=value;return{task:{id,status:"queued"}};}},logger:{info(){},error(){}}}),body=JSON.stringify({expectedVersion:12,runtimeBudgetMinutes:60,candidatePaths:["assets/voice-input.js","test/api.test.js"]}),headers={"content-type":"application/json"},denied=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/replan-discovery-only",headers,body}),denied);assert.equal(denied.statusCode,401);assert.equal(taskId,undefined);const allowed=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/replan-discovery-only",headers:{...headers,authorization:"Bearer local-only"},body}),allowed);assert.equal(allowed.statusCode,200);assert.equal(taskId,"exact-task");assert.equal(input.expectedVersion,12);});
test("implementation-plan recovery is exact-task and scoped-worker protected",async()=>{let taskId,input;const app=createApi({agent:{tools:{list(){return[];}},async run(){throw new Error("unused");}},config:{allowedOrigins:[],maxBodyBytes:64*1024,localWorkerToken:"local-only"},storage:{provider:"memory",durable:false},initialize:async()=>{},ownerId:"owner",selfDevelopment:{async recoverImplementationPlan(id,value){taskId=id;input=value;return{task:{id,status:"queued"}};}},logger:{info(){},error(){}}}),body=JSON.stringify({expectedVersion:24}),headers={"content-type":"application/json"},denied=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/recover-implementation-plan",headers,body}),denied);assert.equal(denied.statusCode,401);assert.equal(taskId,undefined);const allowed=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/recover-implementation-plan",headers:{...headers,authorization:"Bearer local-only"},body}),allowed);assert.equal(allowed.statusCode,200);assert.equal(taskId,"exact-task");assert.equal(input.expectedVersion,24);});
test("focused-test evidence recovery is exact-task and scoped-worker protected",async()=>{let taskId;const app=createApi({agent:{tools:{list(){return[];}},async run(){throw new Error("unused");}},config:{allowedOrigins:[],maxBodyBytes:64*1024,localWorkerToken:"local-only"},storage:{provider:"memory",durable:false},initialize:async()=>{},ownerId:"owner",selfDevelopment:{async recoverFocusedTestEvidence(id){taskId=id;return{task:{id,status:"queued"}};}},logger:{info(){},error(){}}}),body=JSON.stringify({expectedVersion:27}),headers={"content-type":"application/json"},denied=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/recover-focused-test-evidence",headers,body}),denied);assert.equal(denied.statusCode,401);const allowed=response();await app.handle(request({method:"POST",url:"/api/admin/self-development/tasks/exact-task/recover-focused-test-evidence",headers:{...headers,authorization:"Bearer local-only"},body}),allowed);assert.equal(allowed.statusCode,200);assert.equal(taskId,"exact-task");});
