...
test("partial repair-plan recovery replans the exact prior same-task dirty output before mutation",async()=>{
  ...
  async function setup({id="partial-plan",workspaceRoot="C:/controlled/nova-brain",changedFiles=paths.map(path=>({path,hashAlgorithm:"git_sha1",hash:gitHash(contents[path])}))}={}){
    const f=await fixture({currentCommit:NEW_SHA,verifyRemote:async()=>({currentTip:NEW_SHA,ancestors:{[NEW_SHA]:true}}),compareRemoteEvidence:async()=>({})}),templates=[
    ...
    await f.storage.updateAutonomyTask(id,OWNER,{metadata:{...planLifecycleMetadata(task,active),steps:templates,handsContextRecoveryHistory:[{repositoryRoot:"C:/controlled/nova-brain"}]}});
    for(const step of [
  ...
  for(const variant of [
    ...
    {name:"wrong workspace",workspaceRoot:"C:/other/nova-brain"},
  ]){const sample=await setup({id:`partial-${variant.name}`,workspaceRoot:variant.workspaceRoot,changedFiles:variant.changedFiles});await assert.rejects(()=>sample.f.service.recoverHandsWorkingTreeDirty(sample.failed.id,{expectedVersion:sample.failed.stateVersion,workspace:sample.workspace}),error=>error.code==="hands_context_recovery_dirty_unproven");}
});
