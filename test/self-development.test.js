...
test("chat-native canonical intent survives harmless objective paraphrases",async()=>{
  for(const objective of ["Make the Console control accessible.","Ensure the Console control exposes its shortcut.","Provide assistive technology with the Console shortcut hint."]){
    const f=await fixture({structuredIntake:{async specify(){return{version:1,status:"ready",intent:"implementation",objective,acceptanceCriteria:["The shortcut hint is exposed."],constraints:[],explicitPaths:[],focusedTests:[],searchTerms:["New conversation shortcut"],clarificationQuestion:"",specificationHash:createHash("sha256").update(objective).digest("hex")};}}}),result=await f.service.createTrustedIntake("Implement the Nova Console shortcut hint");
...
test("direct callers cannot elevate lexical analysis intent through intake metadata",async()=>{
  ...
  assert.equal(result.request.intent,"analysis_only");assert.equal(result.task.metadata.selfDevelopment.intent,"analysis_only");
});
test("chat-native intake clarification fails closed without creating a task",async()=>{
  const f=await fixture({structuredIntake:{async specify(){return{status:"clarification_required",intent:"implementation",clarificationQuestion:"Which customer account is authorized?"};}}});
