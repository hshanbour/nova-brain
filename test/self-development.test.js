...
test("partial repair-plan recovery replans the exact prior same-task dirty output before mutation",async()=>{
  ...
  async function setup({id="partial-plan",workspaceRoot="C:/controlled/nova-brain",workspaceHead=NEW_SHA,remoteTip=NEW_SHA,changedFiles=paths.map(path=>({path,hashAlgorithm:"git_sha1",hash:gitHash(contents[path])}))}={}){
    ...
    return{f,failed,workspace:{root:workspaceRoot,gitTopLevel:workspaceRoot,head:workspaceHead,clean:false,changedFiles},async attest(overrides={},actorOverrides={}){const workspaceProof={root:workspaceRoot,gitTopLevel:workspaceRoot,repository:"hshanbour/nova-brain",branch:BRANCH,head:workspaceHead,liveTip:remoteTip,clean:false,changedFiles,...overrides};return f.service.attestHandsWorkspace(id,{expectedVersion:failed.stateVersion,runtimeVersion,workerId:"persistent-worker",sourceApplyStepId:"2:apply_patch",workspaceProof,workspaceProofSignature:"signed"},{actorType:"scoped_local_worker",workspaceProof,...actorOverrides});}};
  }
  const exact=await setup(),attested=await exact.attest(),recoveryInput={expectedVersion:exact.failed.stateVersion,runtimeVersion,workspace:exact.workspace},recovered=await exact.f.service.recoverHandsWorkingTreeDirty(exact.failed.id,recoveryInput);
