...
test("Git PATH recovery preserves non-adjacent focused success across a full-test-only continuation",async()=>{
  const f=await fixture(),files=[{path:"assets/voice-input.js",operation:"replace",expectedContent:"old\n",content:"new\n"}],plain={files,evidencePaths:["assets/voice-input.js"],planHash:"repair-plan"},templates=Array.from({length:12},(_,index)=>({type:"inspect_repo",input:{arguments:{}},idempotencyIdentity:`history-${index+1}`}));
  templates[9]={type:"run_full_tests",input:{arguments:{}},idempotencyIdentity:"reopened-full"};templates[10]={type:"plan_repair",input:{arguments:{}},idempotencyIdentity:"failed-repair"};templates[11]={type:"inspect_diff",input:{arguments:{}},idempotencyIdentity:"remaining"};
  ...
  task=await f.storage.updateAutonomyTask(task.id,OWNER,{metadata:{...planLifecycleMetadata(task,plan),steps:templates,fullTestRepairHistory:[{iteration:1,fingerprint:diagnostics.fingerprint,failedStepId:"10:run_full_tests"}]}});
  for(const[ordinal,type,status,result,errorCode]of[[3,"run_focused_tests","completed",{ok:true},null],[10,"run_full_tests","failed",{code:"test_failed",diagnostics},"test_failed"],[11,"plan_repair","failed",{diagnostics:{validationIssues:["focused_test_evidence_required"]}},"implementation_scope_violation"]])await f.storage.recordAutonomyStep({taskId:task.id,stepId:`${ordinal}:${type}`,stepType:type,capability:type.includes("test")?"test_local":"reasoning",operationFingerprint:`fp-${ordinal}`,status,result,errorCode});
  const failed=await f.storage.updateAutonomyTask(task.id,OWNER,{status:"failed",currentStep:10,errorCode:"implementation_scope_violation"}),recovered=await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:failed.stateVersion});
  assert.equal(recovered.task.status,"waiting_for_worker");assert.deepEqual(recovered.recovery.retiredRepairHistoryStepIds,["10:run_full_tests"]);assert.equal(recovered.task.metadata.steps[templates.length].type,"run_full_tests");
});
