...
test("partial repair-plan recovery replans the exact prior same-task dirty output before mutation",async()=>{
  ...
  async function setup({id="partial-plan",workspaceRoot="C:/controlled/nova-brain",workspaceHead=NEW_SHA,remoteTip=NEW_SHA,changedFiles=paths.map(path=>({path,hashAlgorithm:"git_sha1",hash:gitHash(contents[path])}))}={}){
    const f=await fixture({currentCommit:runtimeVersion,runtimeVersion,verifyRemote:async()=>({currentTip:remoteTip,ancestors:{[NEW_SHA]:true,[workspaceHead]:remoteTip===workspaceHead}}),compareRemoteEvidence:async()=>({})}),templates=[
    ...
    return{f,failed,workspace:{root:workspaceRoot,gitTopLevel:workspaceRoot,head:workspaceHead,clean:false,changedFiles}};
  }
  const exact=await setup(),recoveryInput={expectedVersion:exact.failed.stateVersion,runtimeVersion,workspace:exact.workspace},recovered=await exact.f.service.recoverHandsWorkingTreeDirty(exact.failed.id,recoveryInput);
