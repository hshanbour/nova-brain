...
test("workspace attestation accepts only an exact immediately post-apply focused-test failure",async()=>{
  ...
  async function setup({id="post-apply-attestation",failedFile=paths[1],lineage=lineageEntries,workspaceFiles=changedFiles,extraStep=null,explicitSourceApply=true,applyStatus="completed",lineageContainer="apply"}={}){
...
test("current rejected focused-test evidence shape reopens only the exact consumed-extension continuation",async()=>{
  let sequence=0;
  const setup=async({diagnosticsPatch={},taskPatch={},metadataPatch={},extensionMode="consumed",extensionPatch={},approvalPatch={},exactVersion=false}={})=>{
