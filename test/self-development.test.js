...
async function historicalDivergedDeliveryFixture({remoteTip="9".repeat(40),baselinePresent=true,taskId="selfdev_10721df97b8cbc63c70d4171f6f4a440",approvalId="fb4e62f7-9189-4151-ac11-c620e934d3aa",approvalArguments,taskOverrides={},metadataOverrides={}}={}) {
  const f=await fixture({verifyRemote:async ({requiredAncestors})=>({currentTip:remoteTip,ancestors:Object.fromEntries(requiredAncestors.map(sha=>[sha,baselinePresent]))})}),reviewedSha="5818ce4a8b0eb13285971cfcede009c7ae0d5aad",paths=["assets/voice-input.js","test/composer-dictation.test.js","test/voice-input.test.js"],reviewedChangeSet={commitSha:reviewedSha,reviewHash:"f1ed0eddac728d408160c564c0944cc506630d1de2da611e980672684c61dd10",allowedPaths:paths,entries:paths.map((path,index)=>({path,status:"committed",contentHash:String(index+1).repeat(40)}))},templates=Array.from({length:308},(_,index)=>({type:index===307?"review_commit":"historical",input:{},idempotencyIdentity:`historical:${index+1}`})),repository="hshanbour/nova-brain",historyBinding={taskId,approvalId,repository,branch:BRANCH,commitSha:reviewedSha};
  await f.storage.createAutonomyTask({id:taskId,ownerId:OWNER,projectId:"nova-brain",title:"historical",objective:"historical",taskType:"self_development",branch:BRANCH,startingCommit:reviewedSha,maxSteps:30,maxRuntimeMinutes:60,metadata:{selfDevelopment:{repository},steps:templates,approvedDeliveryRuntime:{...historyBinding,consumed:false},approvedDeliveryRecoveryHistory:[historyBinding],...metadataOverrides}});
  const approval=await f.storage.createApproval({id:approvalId,ownerId:OWNER,projectId:"nova-brain",runId:taskId,tool:"git_push",arguments:approvalArguments||{repository,branch:BRANCH,commitSha:reviewedSha,approvedStateVersion:264}});await f.storage.decideApproval(approval.id,OWNER,"approved");
  await f.storage.recordAutonomyStep({taskId,stepId:"308:review_commit",stepType:"review_commit",capability:"repo_read_remote",operationFingerprint:"review",status:"completed",result:{commitSha:reviewedSha,reviewedChangeSet}});await f.storage.recordAutonomyStep({taskId,stepId:"309:push",stepType:"push",capability:"github_write",operationFingerprint:"push",status:"failed",attempt:2,errorCode:"push_failed",result:{message:"Public push failed."}});
  let task=await f.storage.getAutonomyTask(taskId,OWNER);while(task.stateVersion<279)task=await f.storage.updateAutonomyTask(taskId,OWNER,{},task.stateVersion);task=await f.storage.updateAutonomyTask(taskId,OWNER,{status:"failed",currentStep:308,currentPhase:"review_commit",currentCommit:reviewedSha,errorCode:"push_failed",approvalState:{approvalId,approved:true,tool:"git_push",repository,branch:BRANCH,commitSha:reviewedSha,approvedStateVersion:264,deliveryStateVersion:277},...taskOverrides,metadata:{...task.metadata,...metadataOverrides}},task.stateVersion);
  return {...f,task,reviewedChangeSet,approvalId,reviewedSha,remoteTip};
}

test("exact historical divergent delivery opens only deterministic integration, review, and new approval boundary",async()=>{
  const f=await historicalDivergedDeliveryFixture(),result=await f.service.recoverDivergedApprovedDeliveryIntegration(f.task.id,{expectedVersion:280}),added=result.task.metadata.steps.slice(308);
  assert.equal(result.task.stateVersion,281);assert.equal(result.task.status,"waiting_for_worker");assert.equal(result.task.approvalState,null);assert.deepEqual(added.map(step=>step.type),["integrate_commit","review_commit","push"]);assert.equal(added[0].input.arguments.firstParentSha,f.remoteTip);assert.equal(added[0].input.arguments.secondParentSha,f.reviewedSha);assert.equal(added[1].input.arguments.firstParentSha,f.remoteTip);assert.equal(added[1].input.arguments.secondParentSha,f.reviewedSha);assert.equal(result.task.metadata.activeContinuation.startStep,308);assert.equal(result.task.metadata.activeContinuation.maxSteps,3);assert.equal(result.task.metadata.activeContinuation.runtimeMinutes,10);assert.equal(result.task.metadata.supersededDeliveryApproval.approvalId,f.approvalId);assert.equal(result.task.metadata.approvedDeliveryRuntime,null);assert.equal((await f.storage.getApproval(f.approvalId,OWNER)).status,"approved");assert.equal((await f.service.recoverDivergedApprovedDeliveryIntegration(f.task.id,{expectedVersion:280})).idempotent,true);
});

test("divergent delivery integration recovery fails closed for stale shape, remote tip, reviewed commit, or consumed approval",async()=>{
  for(const options of [
    {taskOverrides:{status:"queued"}},
    {taskOverrides:{errorCode:"unexpected_error"}},
    {taskOverrides:{currentCommit:"a".repeat(40)}},
    {metadataOverrides:{approvedDeliveryRuntime:{consumed:true}}},
  ]){const f=await historicalDivergedDeliveryFixture(options);await rejects(f.service.recoverDivergedApprovedDeliveryIntegration(f.task.id,{expectedVersion:280}),"diverged_delivery_integration_recovery_precondition_failed");}
  const changed=await historicalDivergedDeliveryFixture({remoteTip:"523386be918a4072e7990f559e6870f5c652498a",baselinePresent:false});await rejects(changed.service.recoverDivergedApprovedDeliveryIntegration(changed.task.id,{expectedVersion:280}),"diverged_delivery_integration_remote_changed");
  const alteredReview=await historicalDivergedDeliveryFixture();await alteredReview.storage.updateAutonomyStep(alteredReview.task.id,"308:review_commit",{result:{commitSha:alteredReview.reviewedSha,reviewedChangeSet:{...alteredReview.reviewedChangeSet,reviewHash:"0".repeat(64)}}});await rejects(alteredReview.service.recoverDivergedApprovedDeliveryIntegration(alteredReview.task.id,{expectedVersion:280}),"diverged_delivery_integration_recovery_precondition_failed");
  const substituted=await historicalDivergedDeliveryFixture({taskOverrides:{approvalState:{approvalId:"substituted",approved:true,tool:"git_push",repository:"hshanbour/nova-brain",branch:BRANCH,commitSha:"5818ce4a8b0eb13285971cfcede009c7ae0d5aad"}}});await rejects(substituted.service.recoverDivergedApprovedDeliveryIntegration(substituted.task.id,{expectedVersion:280}),"diverged_delivery_integration_recovery_precondition_failed");
  const exact=await historicalDivergedDeliveryFixture();await rejects(exact.service.recoverDivergedApprovedDeliveryIntegration(exact.task.id,{expectedVersion:279}),"diverged_delivery_integration_recovery_invalid");await rejects(exact.service.recoverDivergedApprovedDeliveryIntegration(exact.task.id,{expectedVersion:280,firstParentSha:"523386be918a4072e7990f559e6870f5c652498a"}),"diverged_delivery_integration_recovery_invalid");await rejects(exact.service.recoverDivergedApprovedDeliveryIntegration("wrong-task",{expectedVersion:280}),"task_not_found");
});

test("historical divergent delivery accepts only the exact repository-less legacy approval with unanimous repository bindings",async()=>{
  const absent={branch:BRANCH,commitSha:"5818ce4a8b0eb13285971cfcede009c7ae0d5aad",approvedStateVersion:264},correct={repository:"hshanbour/nova-brain",...absent};
  for(const approvalArguments of [absent,correct]){const f=await historicalDivergedDeliveryFixture({approvalArguments});assert.equal((await f.service.recoverDivergedApprovedDeliveryIntegration(f.task.id,{expectedVersion:280})).task.stateVersion,281);}
  for(const options of [
    {approvalArguments:{...correct,repository:"wrong/repository"}},
    {approvalArguments:absent,taskOverrides:{approvalState:{approvalId:"fb4e62f7-9189-4151-ac11-c620e934d3aa",approved:true,tool:"git_push",repository:"wrong/repository",branch:BRANCH,commitSha:"5818ce4a8b0eb13285971cfcede009c7ae0d5aad",approvedStateVersion:264,deliveryStateVersion:277}}},
    {approvalArguments:absent,metadataOverrides:{approvedDeliveryRuntime:{taskId:"selfdev_10721df97b8cbc63c70d4171f6f4a440",approvalId:"fb4e62f7-9189-4151-ac11-c620e934d3aa",repository:"wrong/repository",branch:BRANCH,commitSha:"5818ce4a8b0eb13285971cfcede009c7ae0d5aad",consumed:false}}},
    {approvalArguments:absent,metadataOverrides:{approvedDeliveryRecoveryHistory:[{taskId:"selfdev_10721df97b8cbc63c70d4171f6f4a440",approvalId:"fb4e62f7-9189-4151-ac11-c620e934d3aa",repository:"wrong/repository",branch:BRANCH,commitSha:"5818ce4a8b0eb13285971cfcede009c7ae0d5aad"}]}},
    {approvalId:"different-approval",approvalArguments:absent},
    {taskId:"selfdev_different",approvalArguments:absent},
  ]){const f=await historicalDivergedDeliveryFixture(options);await rejects(f.service.recoverDivergedApprovedDeliveryIntegration(f.task.id,{expectedVersion:280}),"diverged_delivery_integration_recovery_precondition_failed");}
});
...
test("Git PATH contamination is retired without consuming a Nova repair attempt",async()=>{
  const f=await fixture(),files=[{path:"assets/voice-input.js",operation:"replace",expectedContent:"old\n",content:"new\n"}],plain={files,evidencePaths:["assets/voice-input.js"],planHash:"repair-plan"},templates=[{type:"plan_repair"},{type:"apply_patch"},{type:"run_focused_tests"},{type:"run_full_tests"},{type:"plan_repair"},{type:"inspect_diff"},{type:"commit"},{type:"review_commit"}].map((step,index)=>({...step,input:{arguments:{}},idempotencyIdentity:`step-${index+1}`}));
  await f.storage.createAutonomyTask({id:"runner-git-path",ownerId:OWNER,projectId:"nova-brain",title:"Runner",objective:"Composer",taskType:"self_development",branch:BRANCH,startingCommit:SHA,currentCommit:SHA,maxSteps:100,maxRuntimeMinutes:60,metadata:{steps:templates,selfDevelopment:{userGoal:"Implement",acceptanceCriteria:["safe"],repairLimit:2},implementationPlanGenerations:[]}});
  let task=await f.storage.getAutonomyTask("runner-git-path",OWNER),plan={...plain,provenance:bindImplementationPlan({task,plan:plain,evidence:[{path:"assets/voice-input.js",content:"old\n"}]})},diagnostics={version:1,identity:{runner:"node_test",command:"npm:test"},durationMs:100,exitCode:1,errorMessage:"A validated Git executable is unavailable.",stdoutExcerpt:"Error: spawn git ENOENT",stderrExcerpt:"",failedFiles:["test/git-execution.test.js","test/voice-input.test.js"],failedTitles:["git failure","voice failure"],counts:{tests:10,failed:2,passed:8,skipped:0},fingerprint:"git-path-fingerprint"};
  task=await f.storage.updateAutonomyTask(task.id,OWNER,{metadata:{...planLifecycleMetadata(task,plan),steps:templates,fullTestRepairHistory:[{iteration:1,fingerprint:diagnostics.fingerprint,failedStepId:"4:run_full_tests"}]}});
  for(const[ordinal,type,status,result,errorCode]of[[1,"plan_repair","completed",{implementationPlan:plan},null],[2,"apply_patch","completed",{files:["assets/voice-input.js"]},null],[3,"run_focused_tests","completed",{ok:true},null],[4,"run_full_tests","failed",{code:"test_failed",diagnostics},"test_failed"],[5,"plan_repair","failed",{diagnostics:{validationIssues:["focused_test_evidence_required"]}},"implementation_scope_violation"]])await f.storage.recordAutonomyStep({taskId:task.id,stepId:`${ordinal}:${type}`,stepType:type,capability:type.includes("test")?"test_local":"reasoning",operationFingerprint:`fp-${ordinal}`,status,result,errorCode});
  const failed=await f.storage.updateAutonomyTask(task.id,OWNER,{status:"failed",currentStep:4,errorCode:"implementation_scope_violation"}),recovered=await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:failed.stateVersion});
  assert.equal(recovered.task.status,"waiting_for_worker");assert.deepEqual(recovered.task.metadata.fullTestRepairHistory,[]);assert.equal(recovered.recovery.recoveryClass,"test_runner_git_path_reclassification");assert.deepEqual(recovered.recovery.retiredRepairHistoryStepIds,["4:run_full_tests"]);assert.equal(recovered.task.metadata.steps[templates.length].type,"run_full_tests");
});

test("Git PATH recovery preserves non-adjacent focused success and excludes a later repair continuation",async()=>{
  const f=await fixture(),files=[{path:"assets/voice-input.js",operation:"replace",expectedContent:"old\n",content:"new\n"}],plain={files,evidencePaths:["assets/voice-input.js"],planHash:"repair-plan"},templates=Array.from({length:20},(_,index)=>({type:"inspect_repo",input:{arguments:{}},idempotencyIdentity:`history-${index+1}`}));
  templates[9]={type:"run_full_tests",input:{arguments:{}},idempotencyIdentity:"reopened-full"};for(const[index,type]of[[10,"inspect_diff"],[11,"commit"],[12,"review_commit"],[13,"push"],[14,"deploy_preview"],[15,"wait"],[16,"verify_preview"],[17,"summarize"],[18,"plan_repair"],[19,"apply_patch"]])templates[index]={type,input:{arguments:{}},idempotencyIdentity:`tail-${index+1}`};
  await f.storage.createAutonomyTask({id:"runner-git-path-gap",ownerId:OWNER,projectId:"nova-brain",title:"Runner",objective:"Composer",taskType:"self_development",branch:BRANCH,startingCommit:SHA,currentCommit:SHA,maxSteps:100,maxRuntimeMinutes:60,metadata:{steps:templates,selfDevelopment:{userGoal:"Implement",acceptanceCriteria:["safe"],repairLimit:2},implementationPlanGenerations:[]}});
  let task=await f.storage.getAutonomyTask("runner-git-path-gap",OWNER),plan={...plain,provenance:bindImplementationPlan({task,plan:plain,evidence:[{path:"assets/voice-input.js",content:"old\n"}]})},diagnostics={version:1,identity:{runner:"node_test",command:"npm:test"},durationMs:0,exitCode:1,errorMessage:"A validated Git executable is unavailable.",stdoutExcerpt:"Error: spawn git ENOENT",stderrExcerpt:"",failedFiles:["test/git-execution.test.js"],failedTitles:["git failure"],counts:{tests:10,failed:1,passed:9,skipped:0},fingerprint:"git-path-gap"};
  task=await f.storage.updateAutonomyTask(task.id,OWNER,{metadata:{...planLifecycleMetadata(task,plan),steps:templates,fullTestRepairHistory:[{iteration:1,fingerprint:diagnostics.fingerprint,failedStepId:"10:run_full_tests"}],continuationHistory:[{startStep:9,recoveryClass:"test_runner_unavailable_reclassification"}],activeContinuation:{startStep:18,recoveryClass:"structured_full_test_repair"}}});
  for(const[ordinal,type,status,result,errorCode]of[[3,"run_focused_tests","completed",{ok:true},null],[10,"run_full_tests","failed",{code:"test_failed",diagnostics},"test_failed"],[19,"plan_repair","failed",{diagnostics:{validationIssues:["focused_test_evidence_required"]}},"implementation_scope_violation"]])await f.storage.recordAutonomyStep({taskId:task.id,stepId:`${ordinal}:${type}`,stepType:type,capability:type.includes("test")?"test_local":"reasoning",operationFingerprint:`fp-${ordinal}`,status,result,errorCode});
  const failed=await f.storage.updateAutonomyTask(task.id,OWNER,{status:"failed",currentStep:18,errorCode:"implementation_scope_violation"}),recovered=await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:failed.stateVersion}),added=recovered.task.metadata.steps.slice(templates.length);
  assert.equal(recovered.task.status,"waiting_for_worker");assert.deepEqual(recovered.recovery.retiredRepairHistoryStepIds,["10:run_full_tests"]);assert.deepEqual(added.map(step=>step.type),["run_full_tests","inspect_diff","commit","review_commit","push","deploy_preview","wait","verify_preview","summarize"]);assert.equal(added.some(step=>step.type==="plan_repair"),false);
});

test("post-runner genuine failure reopens only its evidence-bound repair continuation",async()=>{
  const f=await fixture(),files=[{path:"assets/voice-input.js",operation:"replace",expectedContent:"old\n",content:"new\n"}],plain={files,evidencePaths:["assets/voice-input.js"],planHash:"repair-plan"},templates=[{type:"inspect_repo"},{type:"inspect_repo"},{type:"run_focused_tests"},{type:"run_full_tests"},{type:"plan_repair"},{type:"apply_patch"},{type:"run_focused_tests"},{type:"run_full_tests"},{type:"inspect_diff"},{type:"commit"},{type:"review_commit"}].map((step,index)=>({...step,input:{arguments:{}},idempotencyIdentity:`repair-${index+1}`}));
  await f.storage.createAutonomyTask({id:"runner-repair-evidence",ownerId:OWNER,projectId:"nova-brain",title:"Runner",objective:"Composer",taskType:"self_development",branch:BRANCH,startingCommit:SHA,currentCommit:SHA,maxSteps:100,maxRuntimeMinutes:60,metadata:{steps:templates,selfDevelopment:{userGoal:"Implement",acceptanceCriteria:["safe"],repairLimit:2},implementationPlanGenerations:[]}});
  let task=await f.storage.getAutonomyTask("runner-repair-evidence",OWNER),plan={...plain,provenance:bindImplementationPlan({task,plan:plain,evidence:[{path:"assets/voice-input.js",content:"old\n"}]})},diagnostics={version:1,identity:{runner:"node_test",command:"npm:test"},durationMs:100,exitCode:1,errorMessage:"Assertion failed",stdoutExcerpt:"test failure",stderrExcerpt:"",failedFiles:["test/voice-input.test.js"],failedTitles:["voice failure"],counts:{tests:10,failed:1,passed:9,skipped:0},fingerprint:"genuine-failure"};
  task=await f.storage.updateAutonomyTask(task.id,OWNER,{metadata:{...planLifecycleMetadata(task,plan),steps:templates,fullTestRepairHistory:[{iteration:1,fingerprint:diagnostics.fingerprint,failedStepId:"4:run_full_tests"}],testRunnerInfrastructureRecoveryHistory:[{recoveryClass:"test_runner_git_path_reclassification",fromStateVersion:0}]}});
  for(const[ordinal,type,status,result,errorCode]of[[3,"run_focused_tests","completed",{ok:true},null],[4,"run_full_tests","failed",{code:"test_failed",diagnostics},"test_failed"],[5,"plan_repair","failed",{diagnostics:{validationIssues:["focused_test_evidence_required"]}},"implementation_scope_violation"]])await f.storage.recordAutonomyStep({taskId:task.id,stepId:`${ordinal}:${type}`,stepType:type,capability:type.includes("test")?"test_local":"reasoning",operationFingerprint:`fp-${ordinal}`,status,result,errorCode});
  const failed=await f.storage.updateAutonomyTask(task.id,OWNER,{status:"failed",currentStep:4,errorCode:"implementation_scope_violation"}),recovered=await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:failed.stateVersion}),added=recovered.task.metadata.steps.slice(templates.length);
  assert.equal(recovered.recovery.recoveryClass,"post_runner_repair_evidence_rebind");assert.equal(recovered.task.status,"queued");assert.deepEqual(recovered.recovery.retiredRepairHistoryStepIds,[]);assert.deepEqual(recovered.task.metadata.fullTestRepairHistory,[{iteration:1,fingerprint:diagnostics.fingerprint,failedStepId:"4:run_full_tests"}]);assert.deepEqual(added.map(step=>step.type),templates.slice(4).map(step=>step.type));
  const historical=await f.storage.updateAutonomyTask(task.id,OWNER,{status:"waiting_for_worker"},recovered.task.stateVersion),rebound=await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:historical.stateVersion});assert.equal(rebound.recovery.recoveryClass,"post_runner_reasoning_dispatch_rebind");assert.equal(rebound.task.status,"queued");assert.equal(rebound.task.currentStep,historical.currentStep);assert.equal(rebound.task.metadata.steps.length,historical.metadata.steps.length);assert.equal((await f.service.recoverTestRunnerInfrastructure(task.id,{expectedVersion:historical.stateVersion})).idempotent,true);
});
...
test("workspace attestation accepts only an exact immediately post-apply focused-test failure",async()=>{
  const runtimeVersion="d".repeat(40),repository="hshanbour/nova-brain",root="C:/controlled/nova-brain",paths=["assets/console.js","test/composer-voice-console.integration.test.js"],contents=Object.fromEntries(paths.map((path,index)=>[path,`nova repair ${index}\n`]));
  ...
  async function setup({id="post-apply-attestation",failedFile=paths[1],lineage=lineageEntries,workspaceFiles=changedFiles,extraStep=null}={}){
    ...
    const failed=await f.storage.updateAutonomyTask(id,OWNER,{status:"failed",currentStep:5,currentPhase:"apply_patch",errorCode:"test_failed"}),proof={root,gitTopLevel:root,repository,branch:BRANCH,head:NEW_SHA,liveTip:NEW_SHA,clean:false,changedFiles:workspaceFiles};
