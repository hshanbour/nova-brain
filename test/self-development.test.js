...
async function historicalDivergedDeliveryFixture({remoteTip="9".repeat(40),baselinePresent=true,taskOverrides={},metadataOverrides={}}={}) {
  const f=await fixture({verifyRemote:async ({requiredAncestors})=>({currentTip:remoteTip,ancestors:Object.fromEntries(requiredAncestors.map(sha=>[sha,baselinePresent]))})}),taskId="selfdev_10721df97b8cbc63c70d4171f6f4a440",reviewedSha="5818ce4a8b0eb13285971cfcede009c7ae0d5aad",approvalId="fb4e62f7-9189-4151-ac11-c620e934d3aa",paths=["assets/voice-input.js","test/composer-dictation.test.js","test/voice-input.test.js"],reviewedChangeSet={commitSha:reviewedSha,reviewHash:"f1ed0eddac728d408160c564c0944cc506630d1de2da611e980672684c61dd10",allowedPaths:paths,entries:paths.map((path,index)=>({path,status:"committed",contentHash:String(index+1).repeat(40)}))},templates=Array.from({length:308},(_,index)=>({type:index===307?"review_commit":"historical",input:{},idempotencyIdentity:`historical:${index+1}`}));
  await f.storage.createAutonomyTask({id:taskId,ownerId:OWNER,projectId:"nova-brain",title:"historical",objective:"historical",taskType:"self_development",branch:BRANCH,startingCommit:reviewedSha,maxSteps:30,maxRuntimeMinutes:60,metadata:{selfDevelopment:{repository:"hshanbour/nova-brain"},steps:templates,approvedDeliveryRuntime:{consumed:false}}});
  const approval=await f.storage.createApproval({id:approvalId,ownerId:OWNER,projectId:"nova-brain",runId:taskId,tool:"git_push",arguments:{repository:"hshanbour/nova-brain",branch:BRANCH,commitSha:reviewedSha,approvedStateVersion:264}});await f.storage.decideApproval(approval.id,OWNER,"approved");
  ...
  let task=await f.storage.getAutonomyTask(taskId,OWNER);while(task.stateVersion<279)task=await f.storage.updateAutonomyTask(taskId,OWNER,{},task.stateVersion);task=await f.storage.updateAutonomyTask(taskId,OWNER,{status:"failed",currentStep:308,currentPhase:"review_commit",currentCommit:reviewedSha,errorCode:"push_failed",approvalState:{approvalId,approved:true,tool:"git_push",repository:"hshanbour/nova-brain",branch:BRANCH,commitSha:reviewedSha,approvedStateVersion:264,deliveryStateVersion:277},...taskOverrides,metadata:{...task.metadata,...metadataOverrides}},task.stateVersion);
