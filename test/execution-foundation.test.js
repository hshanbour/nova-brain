...
test("real registry exposes bounded reads local writes and separate push approval",async()=>{const {storage,registry}=await foundation();registerDeveloperTools(registry,{root:process.cwd(),environment:{NOVA_BRAIN_DEVELOPMENT_BRANCH:"feat/nova-brain-mvp-foundation"},storage,ownerId:OWNER_ID});registerSystemTools(registry,{storage,ownerId:OWNER_ID});const tools=registry.list();assert.ok(tools.some((tool)=>tool.name==="repo_read"&&tool.available));assert.ok(tools.some((tool)=>tool.name==="repo_apply_patch"&&tool.available));assert.ok(tools.some((tool)=>tool.name==="test_run"&&tool.available));assert.equal(tools.find((tool)=>tool.name==="git_push").riskLevel,RISK_LEVELS.SENSITIVE);const result=await registry.execute("repo_read",{path:"package.json"});assert.match(result.content,/nova-brain/);await assert.rejects(()=>registry.execute("repo_read",{path:"../outside"}),error=>error.code==="path_traversal");});

test("serverless registry exposes remote discovery while local mutation and tests stay unavailable",async()=>{const {registry}=await foundation();registerDeveloperTools(registry,{environment:{VERCEL:"1"}});const tools=registry.list();for(const name of ["repo_list","repo_read","repo_search","git_status","repo_diff"])assert.equal(tools.find((tool)=>tool.name===name).available,true);for(const name of ["repo_apply_patch","git_commit","test_run","test_run_full"])assert.equal(tools.find((tool)=>tool.name===name).available,false);});
...
test("the real serverless executable set serializes safely and reaches a normal OpenAI response", async () => {
  ...
  for(const name of ["repo_list","repo_read","repo_search","git_status","repo_diff","hands_repair_plan","project_list","memory_remember","memory_forget"])assert.ok(executable.some(tool=>tool.name===name));
  ...
  assert.equal(sent.tools.some(({ name }) => name === "repo_search"), true);
});
...
test("configured repository writes remain bound away from main and secrets are redacted in approvals",async()=>{const {storage,registry}=await foundation();registerDeveloperTools(registry,{root:process.cwd(),environment:{NOVA_BRAIN_DEVELOPMENT_BRANCH:"feat/nova-brain-mvp-foundation"},storage,ownerId:OWNER_ID});await assert.rejects(()=>registry.execute("repo_apply_patch",{branch:"main",files:[{path:"README.md",content:"x"}]}),error=>error.code==="branch_not_allowed");registry.register({name:"secret_action",riskLevel:RISK_LEVELS.SENSITIVE,available:true,async execute(){return true;}});try{await registry.execute("secret_action",{apiKey:"must-not-appear"});}catch{}const approval=(await storage.listApprovals(OWNER_ID))[0];assert.equal(approval.arguments.apiKey,"[REDACTED]");assert.equal(JSON.stringify(approval).includes("must-not-appear"),false);});
