...
Only the returned owner-facing assistant message is sent to `POST /api/voice/speech`. Server-side sanitisation removes markup, URLs, emoji noise, code blocks, JSON-shaped output, and internal/tool-labelled lines before bounded semantic chunking. Before Voice Mode starts, readiness queries ElevenLabs' account-scoped model list and the configured owner voice metadata, caches the safe result briefly, and exposes only compatibility status—not provider credentials or the voice ID. Nova prefers the account-supported `eleven_v3_conversational` model and visibly reports if it must select a supported multilingual fallback. The live ElevenLabs REST path rejects request-stitching context for conversational v3, so that model receives only `text` and `model_id`; Nova preserves continuity through deterministic semantic ordering rather than incompatible `previous_text`/`next_text` fields. The API requests `mp3_44100_128`, streams complete MP3 chunks as NDJSON with backpressure, and the browser plays the first chunk while prefetching the next. This avoids waiting for the whole reply while retaining broadly supported Blob/object-URL playback rather than depending on browser-specific decoding of arbitrary MP3 transport fragments. Keys and the voice identifier remain server-only; audio is `no-store`, ephemeral, and discarded after playback.
