...
export function createSpeakerExtractor({ config, fetchImpl = fetch, clock = Date.now } = {}) {
  ...
  async function request(path, body, { signal } = {}) {
    ...
    try {
      response = await fetchImpl(`${speaker.endpoint.replace(/\/$/, "")}${path}`, {
        method: "POST", headers: { Authorization: `Bearer ${speaker.token}`, "Content-Type": "application/json", Accept: "application/json" },
  ...
  return Object.freeze({
    ...
    async extract({ audioBase64, mimeType, durationSeconds }, { signal } = {}) {
      ...
      const startedAt = clock(); const result = await request("/embed", { audioBase64, mimeType, durationSeconds: duration }, { signal });
