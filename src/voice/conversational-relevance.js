...
export function classifyConversationalRelevance({ transcript, speaker, context = {} } = {}) {
  ...
  const strongDirectAddress = explicitlyAddressed && Boolean(addressRemainder) && (directQuestion || novaDirectedLanguage || conversationalGreeting || interruptionIntent || ownerNovaImperative || (context.interruption !== true && substantiveDirectAddress));
  ...
  if (!owner && context.voice_session_engaged === true && (selfIdentityClaim || identityQuestion)) return decision(AUDIO_RELEVANCE.ADDRESSED, true, "restricted_identity_conversation_in_active_session", .84);
