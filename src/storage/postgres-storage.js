...
export function createPostgresStorage({ connectionString }) {
  ...
  const storage = {
    ...
    async updateAutonomyTask(id, ownerId, patch, expectedVersion) {
      ...
      const versionParameter=params.length;
      return autonomyTaskRow(
        (
          await run(
    ...
    async migrateAutonomyTask(input) {
      ...
      const rows = await run(
        `WITH updated AS (UPDATE nova_autonomy_tasks SET status='queued',starting_commit=$6,current_commit=$6,max_steps=GREATEST(max_steps,$7),max_runtime_minutes=$8,started_at=now(),completed_at=NULL,next_run_at=now(),blocked_reason=NULL,error_code=NULL,lease_owner=NULL,lease_token=NULL,lease_expires_at=NULL,metadata=metadata||$9::jsonb,state_version=state_version+1,updated_at=now() WHERE id=$1 AND owner_id=$2 AND branch=$3 AND current_commit=$4 AND state_version=$5::bigint AND (lease_token IS NULL OR lease_expires_at<=now()) RETURNING *), audited AS (INSERT INTO nova_activity_events (id,owner_id,project_id,run_id,action,status,summary,metadata) SELECT gen_random_uuid(),owner_id,project_id,id,'autonomy_task_migrated','completed','Repaired an allowlisted Worker task continuation.',$10::jsonb FROM updated RETURNING id) SELECT * FROM updated`,
        [
          input.taskId,
          input.ownerId,
    ...
    async recoverExpiredAutonomyTask(input) {
      const recoveryKey=input.recoveryMetadataKey||"postAttestationRecovery",metadata=JSON.stringify({steps:input.plan,requiredCapability:input.requiredCapability||"vercel_preview",[recoveryKey]:{previousStatus:"expired",previousErrorCode:"max_runtime_reached",previousStartedAt:input.previousStartedAt,previousCompletedAt:input.previousCompletedAt,recoveredAt:new Date().toISOString()}}),audit=JSON.stringify({taskId:input.taskId,previousStatus:"expired",previousErrorCode:"max_runtime_reached",newRuntimeMinutes:input.runtimeMinutes,actorType:input.actorType,expectedVersion:input.expectedVersion,newVersion:input.expectedVersion+1,result:"completed"});
      const rows=await run(`WITH updated AS (UPDATE nova_autonomy_tasks SET status='queued',current_step=$10,current_phase='post_attestation_recovery',max_steps=GREATEST(max_steps,$6),max_runtime_minutes=$7,started_at=now(),completed_at=NULL,next_run_at=now(),blocked_reason=NULL,error_code=NULL,metadata=metadata||$8::jsonb,state_version=state_version+1,updated_at=now() WHERE id=$1 AND owner_id=$2 AND branch=$3 AND current_commit=$4 AND state_version=$5::bigint AND status='expired' AND error_code='max_runtime_reached' AND lease_owner IS NULL AND lease_token IS NULL AND lease_expires_at IS NULL RETURNING *), audited AS (INSERT INTO nova_activity_events (id,owner_id,project_id,run_id,action,status,summary,metadata) SELECT gen_random_uuid(),owner_id,project_id,id,$11,'completed',$12,$9::jsonb FROM updated RETURNING id) SELECT * FROM updated`,[input.taskId,input.ownerId,input.expectedBranch,input.expectedCommit,input.expectedVersion,input.plan.length+1,input.runtimeMinutes,metadata,audit,input.resumeStep??9,input.activityAction||"autonomy_post_attestation_expiry_recovered",input.activitySummary||"Recovered the exact expired post-attestation verification continuation."]);return autonomyTaskRow(rows[0]);
    },
    ...
    async releaseAutonomyLease(id, ownerId, leaseToken) {
      return (
        (
          await run(
            "UPDATE nova_autonomy_tasks SET lease_owner=NULL,lease_token=NULL,lease_expires_at=NULL,state_version=state_version+1,updated_at=now() WHERE id=$1 AND owner_id=$2 AND lease_token=$3 RETURNING id",
