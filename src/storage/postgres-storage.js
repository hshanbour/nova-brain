...
export function createPostgresStorage({ connectionString }) {
  ...
  const storage = {
    ...
    async sumVoiceBenchmarkCost(ownerId){return Number((await run("SELECT COALESCE(SUM(estimated_cost_usd),0) AS total FROM nova_voice_benchmark_results WHERE owner_id=$1",[ownerId]))[0]?.total||0);}
  };
