...
export function createPostgresStorage({ connectionString }) {
  ...
  const storage = {
    ...
    async recoverExpiredAutonomyTask(input) {
      const metadata=JSON.stringify({steps:input.plan,requiredCapability:"vercel_preview",postAttestationRecovery:{previousStatus:"expired",previousErrorCode:"max_runtime_reached",previousStartedAt:input.previousStartedAt,previousCompletedAt:input.previousCompletedAt,recoveredAt:new Date().toISOString()}}),audit=JSON.stringify({taskId:input.taskId,previousStatus:"expired",previousErrorCode:"max_runtime_reached",newRuntimeMinutes:input.runtimeMinutes,actorType:input.actorType,expectedVersion:input.expectedVersion,newVersion:input.expectedVersion+1,result:"completed"});
      const rows=await run(`WITH updated AS (UPDATE nova_autonomy_tasks SET status='queued',current_step=9,current_phase='post_attestation_recovery',max_steps=GREATEST(max_steps,$6),max_runtime_minutes=$7,started_at=now(),completed_at=NULL,next_run_at=now(),blocked_reason=NULL,error_code=NULL,metadata=metadata||$8::jsonb,state_version=state_version+1,updated_at=now() WHERE id=$1 AND owner_id=$2 AND branch=$3 AND current_commit=$4 AND state_version=$5::bigint AND status='expired' AND error_code='max_runtime_reached' AND lease_owner IS NULL AND lease_token IS NULL AND lease_expires_at IS NULL RETURNING *), audited AS (INSERT INTO nova_activity_events (id,owner_id,project_id,run_id,action,status,summary,metadata) SELECT gen_random_uuid(),owner_id,project_id,id,'autonomy_post_attestation_expiry_recovered','completed','Recovered the exact expired post-attestation verification continuation.',$9::jsonb FROM updated RETURNING id) SELECT * FROM updated`,[input.taskId,input.ownerId,input.expectedBranch,input.expectedCommit,input.expectedVersion,input.plan.length+1,input.runtimeMinutes,metadata,audit]);return autonomyTaskRow(rows[0]);
    },
