...
export function createInMemoryStorage({ clock = () => new Date() } = {}) {
  const owners = new Map(); const projects = new Map(); const conversations = new Map(); const messages = new Map(); const memories = new Map(); const speakerProfiles = new Map(); const anonymousSpeakerProfiles = new Map(); const voiceUtterances = new Map(); const runs = new Map(); const autonomyTasks=new Map();const autonomySteps=new Map();const autonomyLocks=new Map(); const approvals = new Map(); const activity = []; const benchmarkSessions = new Map(); const benchmarkResults = new Map(); const benchmarkBudgets = new Map();
  let sequence = 0;
  ...
  return Object.freeze({
    ...
    async createAutonomyTask(input){const timestamp=now(clock);const task={id:input.id||randomUUID(),ownerId:input.ownerId,projectId:input.projectId||null,title:input.title,objective:input.objective,taskType:input.taskType||"developer",status:"queued",priority:input.priority||0,currentPhase:"queued",currentStep:0,maxSteps:input.maxSteps||30,maxRetries:input.maxRetries??3,maxRuntimeMinutes:input.maxRuntimeMinutes||30,branch:input.branch||null,startingCommit:input.startingCommit||null,currentCommit:input.startingCommit||null,checkpoint:{completedSteps:[],pendingStep:null,findings:[]},approvalState:null,blockedReason:null,resultSummary:null,errorCode:null,nextRunAt:input.nextRunAt||timestamp,metadata:copy(input.metadata||{}),leaseOwner:null,leaseToken:null,leaseExpiresAt:null,retryCount:0,repairIteration:0,createdAt:timestamp,startedAt:null,updatedAt:timestamp,completedAt:null};autonomyTasks.set(task.id,task);return copy(task);},
    async getAutonomyTask(id,ownerId){const task=autonomyTasks.get(id);return copy(task?.ownerId===ownerId?task:null);},
    async listAutonomyTasks(ownerId,{status,limit=50}={}){return[...autonomyTasks.values()].filter(x=>x.ownerId===ownerId&&(!status||x.status===status)).sort((a,b)=>b.priority-a.priority||a.createdAt.localeCompare(b.createdAt)).slice(0,limit).map(copy);},
    async updateAutonomyTask(id,ownerId,patch){const current=autonomyTasks.get(id);if(!current||current.ownerId!==ownerId)return null;const updated={...current,...copy(patch),id,ownerId,createdAt:current.createdAt,updatedAt:now(clock)};autonomyTasks.set(id,updated);return copy(updated);},
    async claimAutonomyTask({ownerId,workerId,capabilities,leaseMs=30000,idempotencyKey}){const timestamp=clock();for(const task of autonomyTasks.values())if(task.ownerId===ownerId&&task.leaseExpiresAt&&new Date(task.leaseExpiresAt)<=timestamp&&["running","planning","retrying"].includes(task.status)){task.status="queued";task.leaseOwner=null;task.leaseToken=null;task.leaseExpiresAt=null;}const eligible=[...autonomyTasks.values()].filter(t=>t.ownerId===ownerId&&(["queued","retrying"].includes(t.status)||(t.status==="waiting"&&t.nextRunAt))&&(!t.nextRunAt||new Date(t.nextRunAt)<=timestamp)&&(!t.metadata?.requiredCapability||capabilities.includes(t.metadata.requiredCapability))).sort((a,b)=>b.priority-a.priority||a.createdAt.localeCompare(b.createdAt))[0];if(!eligible)return null;if(eligible.leaseToken&&eligible.metadata?.claimKey===idempotencyKey)return copy(eligible);eligible.status=eligible.startedAt?"running":"planning";eligible.startedAt||=timestamp.toISOString();eligible.leaseOwner=workerId;eligible.leaseToken=randomUUID();eligible.leaseExpiresAt=new Date(timestamp.getTime()+leaseMs).toISOString();eligible.metadata={...eligible.metadata,claimKey:idempotencyKey};eligible.updatedAt=timestamp.toISOString();return copy(eligible);},
    async releaseAutonomyLease(id,ownerId,leaseToken){const task=autonomyTasks.get(id);if(!task||task.ownerId!==ownerId||task.leaseToken!==leaseToken)return false;task.leaseOwner=null;task.leaseToken=null;task.leaseExpiresAt=null;task.updatedAt=now(clock);return true;},
    async recordAutonomyStep(input){const key=`${input.taskId}:${input.stepId}`;const fingerprint=[...autonomySteps.values()].find(x=>x.taskId===input.taskId&&x.operationFingerprint===input.operationFingerprint);if(fingerprint)return copy(fingerprint);const step={...copy(input),status:input.status||"running",attempt:input.attempt||1,createdAt:now(clock),startedAt:input.startedAt||now(clock),completedAt:input.completedAt||null};autonomySteps.set(key,step);return copy(step);},
    async updateAutonomyStep(taskId,stepId,patch){const key=`${taskId}:${stepId}`,current=autonomySteps.get(key);if(!current)return null;const updated={...current,...copy(patch)};autonomySteps.set(key,updated);return copy(updated);},
    async listAutonomySteps(taskId){return[...autonomySteps.values()].filter(x=>x.taskId===taskId).sort((a,b)=>a.createdAt.localeCompare(b.createdAt)).map(copy);},
    async acquireAutonomyLock({lockKey,taskId,leaseToken,expiresAt}){const current=autonomyLocks.get(lockKey);if(current&&new Date(current.expiresAt)>clock()&&current.taskId!==taskId)return false;autonomyLocks.set(lockKey,{lockKey,taskId,leaseToken,expiresAt});return true;},
    async releaseAutonomyLocks(taskId,leaseToken){let count=0;for(const [key,value]of autonomyLocks)if(value.taskId===taskId&&(!leaseToken||value.leaseToken===leaseToken)){autonomyLocks.delete(key);count++;}return count;},
