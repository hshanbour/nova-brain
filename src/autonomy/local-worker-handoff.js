...
const taskOwnedDirtyLineage=(task,steps,files)=>{
  ...
  const durable=apply.result?.taskOwnedDirtyLineage,applied=new Set(apply.result.files),derived=priorFiles.filter(file=>applied.has(file.path)&&typeof file.content==="string").map(file=>({path:file.path,contentHash:canonicalContentHash(file.content)})),entries=durable?.version===1&&durable.taskId===task.id&&durable.repository===task.metadata?.selfDevelopment?.repository&&durable.branch===task.branch&&durable.currentCommit===task.currentCommit&&durable.sourceApplyStepId===apply.stepId&&Array.isArray(durable.entries)?durable.entries:derived,targetPaths=[...new Set(files.map(file=>file.path))];
  ...
  return Object.freeze({version:1,taskId:task.id,repository:task.metadata?.selfDevelopment?.repository,branch:task.branch,currentCommit:task.currentCommit,sourcePlanStepId:durable?.sourceApplyStepId===apply.stepId?durable.sourcePlanStepId:planStep.stepId,sourceApplyStepId:apply.stepId,activePlanStepId:activePlanStep?.stepId||null,entries});
};
