...
const approvedDeliveryRuntimeWindow=(task,now)=>{const runtime=task.metadata?.approvedDeliveryRuntime,startedMs=new Date(runtime?.startedAt).getTime(),deadlineMs=new Date(runtime?.deadline).getTime(),valid=runtime?.consumed===false&&runtime?.deliveryStateVersion===task.stateVersion&&Number.isFinite(startedMs)&&Number.isFinite(deadlineMs)&&deadlineMs>startedMs;return Object.freeze({scope:"approved_delivery",startedAt:valid?new Date(startedMs).toISOString():null,deadline:valid?new Date(deadlineMs).toISOString():null,elapsedMs:valid?Math.max(0,now.getTime()-startedMs):null,maxRuntimeMs:valid?deadlineMs-startedMs:null,expired:!valid||deadlineMs<=now.getTime(),continuationGenerationId:null});};
...
const taskBoundPatchArguments=(task,args)=>{if(args.branch!==undefined&&args.branch!==task.branch)throw new HandoffError("branch_mismatch","Patch branch does not match the exact task branch.");if(args.currentCommit!==undefined&&args.currentCommit!==task.currentCommit)throw new HandoffError("commit_mismatch","Patch commit does not match the exact task commit.");const planProvenance=assertActiveImplementationPlan(task,args.files);return{branch:task.branch,currentCommit:task.currentCommit,files:args.files,planProvenance};};
...
export function createLocalWorkerHandoff({storage,ownerId,approvedBranch="feat/nova-brain-mvp-foundation",clock=()=>new Date(),leaseMs=120000,deploymentEnvironment="preview"}={}){
  ...
  async function claim(input){
    ...
    if(!planned&&before.approvalState?.approved===true){const approval=await storage.getApproval(before.approvalState.approvalId,ownerId),steps=await storage.listAutonomySteps(before.id);if(isExactApprovedDelivery({task:before,approval,steps,approvedBranch})){exactApprovedDelivery=true;planned={type:"push",input:{tool:"git_push",arguments:{branch:before.branch,commitSha:before.currentCommit}}};}}
