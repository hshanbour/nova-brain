...
const resolvePlan=(value,task)=>{if(Array.isArray(value))return value.map(item=>resolvePlan(item,task));if(value&&typeof value==="object")return Object.fromEntries(Object.entries(value).map(([key,item])=>[key,resolvePlan(item,task)]));const plan=task.metadata?.selfDevelopmentImplementationPlan;if(value==="$IMPLEMENTATION_FILES")return plan?.files;if(value==="$IMPLEMENTATION_TESTS")return plan?.focusedTests;if(value==="$IMPLEMENTATION_PATHS")return plan?.files?.map(file=>file.path);if(value==="$CURRENT_COMMIT")return task.currentCommit;return value;};
