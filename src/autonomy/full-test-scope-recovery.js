...
export const FAILED_FULL_TEST_RETRY_CLASS="owner_approved_failed_full_test_retry";
export const FAILED_FULL_TEST_RETRY_TOOL="self_development_failed_full_test_retry";
const FULL_SCOPE=Object.freeze({historyKey:"fullTestScopeRecoveryHistory",boundaryKey:"fullTestScopeBoundary",recoveryClass:FULL_TEST_SCOPE_RECOVERY_CLASS,tool:FULL_TEST_SCOPE_RECOVERY_TOOL,isRetry:false});
const RETRY_SCOPE=Object.freeze({historyKey:"failedFullTestRetryHistory",boundaryKey:"failedFullTestRetryBoundary",recoveryClass:FAILED_FULL_TEST_RETRY_CLASS,tool:FAILED_FULL_TEST_RETRY_TOOL,isRetry:true});
export function fullTestScopeDescriptor(task){return task?.metadata?.failedFullTestRetryHistory?.length||task?.metadata?.activeContinuation?.recoveryClass===FAILED_FULL_TEST_RETRY_CLASS?RETRY_SCOPE:task?.metadata?.fullTestScopeRecoveryHistory?.length||task?.metadata?.activeContinuation?.recoveryClass===FULL_TEST_SCOPE_RECOVERY_CLASS?FULL_SCOPE:null;}
...
function histories(task,descriptor=fullTestScopeDescriptor(task)||FULL_SCOPE){
  const keys=[...new Set([...fixedHistoryKeys,...Object.keys(task.metadata||{}).filter(key=>/(?:History|Boundary)$/.test(key)&&!["continuationHistory",descriptor.historyKey,descriptor.boundaryKey].includes(key))])].sort();
...
function failedFullTestBoundary(task,steps){
  const history=task.metadata?.fullTestScopeRecoveryHistory,predecessor=history?.[0],active=task.metadata?.activeContinuation,boundary=task.metadata?.fullTestScopeBoundary;
  requireProof(history?.length===1&&predecessor?.recoveryClass===FULL_TEST_SCOPE_RECOVERY_CLASS&&active&&predecessor.consumed===true&&predecessor.authorizationConsumed===true&&predecessor.result==="test_failed"&&predecessor.workerBindingState==="bound"&&WORKER.test(predecessor.workerId||"")&&same(predecessor.activeContinuation,active)&&task.currentStep===active.startStep+1&&Date.parse(predecessor.completedAt)>=Date.parse(active.runtimeStartedAt)&&Date.parse(predecessor.completedAt)<=Date.parse(active.runtimeDeadline),"consumed_failed_full_test_predecessor");
  const failed=steps.find(step=>step.stepId===predecessor.fullTestStepId);
  requireProof(failed?.taskId===task.id&&failed.stepType==="run_full_tests"&&failed.status==="failed"&&failed.attempt===1&&failed.errorCode==="test_failed"&&failed.result?.code==="test_failed"&&same(failed.input,{tool:"test_run_full",arguments:{}})&&same(predecessor.claimedStepIds,[failed.stepId])&&predecessor.resultHash===recoveryHash(failed.result)&&steps.length===predecessor.predecessorStepProofs?.length+1&&!steps.some(step=>ordinal(step)>ordinal(failed)),"exact_failed_full_test_step");
  // Validate the immutable pre-run binding at its historical timestamp. The
  // failed execution is separately checked in full; no durable object is edited
  // and the consumed predecessor never becomes executable again.
  const projected=structuredClone(task);projected.status="running";projected.currentStep=active.startStep;projected.metadata.fullTestScopeRecoveryHistory[0].consumed=false;
  try{validateFullTestScopeEvidence(projected,steps.filter(step=>step.stepId!==failed.stepId),()=>new Date(active.runtimeStartedAt));}catch(error){requireProof(false,`failed_full_test_evidence:${error.safeDiagnostics?.predicate||error.code||"invalid"}`);}
  requireProof(same(boundary,predecessor.boundary)&&same(boundary,{kind:"product_repair_decision",executionAuthorized:false,mutationApplied:false,planHash:predecessor.planHash,planGenerationId:predecessor.planGenerationId,stepId:failed.stepId,errorCode:"test_failed"}),"failed_full_test_boundary");
  const diagnostics=failed.result.diagnostics,counts=diagnostics?.counts,output=`${diagnostics?.stdoutExcerpt||""}\n${diagnostics?.stderrExcerpt||""}`;
  requireProof(diagnostics?.version===1&&diagnostics.identity?.command==="npm:test"&&diagnostics.exitCode===1&&!diagnostics.signal&&HASH.test(diagnostics.fingerprint||"")&&Number.isInteger(counts?.tests)&&counts.tests>0&&Number.isInteger(counts.passed)&&counts.passed>=0&&Number.isInteger(counts.failed)&&counts.failed>0&&counts.skipped===0&&counts.passed+counts.failed===counts.tests&&Array.isArray(diagnostics.failedFiles)&&diagnostics.failedFiles.length>0&&diagnostics.failedFiles.every(path=>typeof path==="string"&&/^test\/[a-z0-9._/-]+\.test\.js$/i.test(path)&&!path.split("/").includes(".."))&&output.includes("ERR_MODULE_NOT_FOUND")&&/Cannot find package ['"]@neondatabase\/serverless['"]/.test(output)&&same(diagnostics.fullTestScopeEvidence,{generationId:active.generationId,planHash:predecessor.planHash,entries:predecessor.entries}),"exact_missing_dependency_failure");
  const plan=task.metadata.selfDevelopmentImplementationPlan,apply=steps.find(step=>step.stepId===predecessor.sourceApplyStepId),focused=steps.find(step=>step.stepId===predecessor.sourceFocusedStepId);
  requireProof(apply?.status==="completed"&&focused?.status==="completed"&&predecessor.approvalArguments.sourceApplyHash===recoveryHash(apply)&&predecessor.approvalArguments.sourceFocusedHash===recoveryHash(focused)&&predecessor.approvalArguments.sourceLineageHash===recoveryHash(apply.result?.taskOwnedDirtyLineage),"preserved_applied_focused_lineage");
  return{predecessor,plan,apply,focused,failed,diagnostics,focusedCounts:structuredClone(predecessor.focusedCounts),entries:structuredClone(predecessor.entries),requiredPaths:structuredClone(predecessor.requiredPaths)};
}

export async function describeFailedFullTestRetry({taskId,input,actor,runtime,storage,ownerId,repository,approvedBranch,runtimeVersion,verifyRemote}){
  const proof=signedRequest({taskId,input,actor,runtimeVersion}),task=await runtime.get(taskId);
  requireProof(task?.id===taskId&&task.taskType==="self_development"&&task.stateVersion===input.expectedVersion&&task.status==="blocked"&&task.currentPhase==="run_full_tests"&&task.errorCode==="test_failed"&&!task.leaseOwner&&!task.leaseToken&&!task.metadata?.localHandoff&&!task.approvalState,"blocked_failed_full_test_task_state");
  requireProof(task.branch===approvedBranch&&task.metadata?.selfDevelopment?.repository===repository&&SHA.test(task.currentCommit||"")&&task.currentCommit===task.startingCommit,"product_identity");
  requireProof(!task.metadata.failedFullTestRetryHistory?.length,"single_use_failed_full_test_retry");
  const steps=await runtime.steps(taskId),state=failedFullTestBoundary(task,steps),{predecessor,plan,apply,focused,failed,diagnostics,focusedCounts,entries,requiredPaths}=state;
  requireProof(input.planHash===plan.planHash,"owner_requested_plan_hash");
  const priorApproval=await storage.getApproval(predecessor.approvalId,ownerId);
  requireProof(priorApproval?.ownerId===ownerId&&priorApproval.projectId===task.projectId&&priorApproval.runId===taskId&&priorApproval.status==="approved"&&priorApproval.tool===FULL_TEST_SCOPE_RECOVERY_TOOL&&same(priorApproval.arguments,predecessor.approvalArguments),"historical_full_test_approval");
  const workspace=proof.workspace;
  requireProof(workspace&&Object.keys(workspace).every(key=>["root","gitTopLevel","repository","branch","head","liveTip","clean","changedFiles"].includes(key))&&recoveryRoot(workspace.root)===predecessor.workspaceRoot&&recoveryRoot(workspace.gitTopLevel)===predecessor.workspaceRoot&&workspace.repository===repository&&workspace.branch===task.branch&&workspace.head===task.currentCommit&&workspace.liveTip===task.currentCommit&&workspace.clean===false&&Array.isArray(workspace.changedFiles)&&workspace.changedFiles.length===requiredPaths.length&&new Set(workspace.changedFiles.map(item=>item?.path)).size===requiredPaths.length&&workspace.changedFiles.every(item=>item&&typeof item.path==="string"&&SHA.test(item.hash||"")&&HASH.test(item.contentHash||""))&&same(entryProof(workspace.changedFiles),entryProof(entries)),"current_workspace");
  requireProof(typeof verifyRemote==="function","remote_verifier");
  const product=await verifyRemote({repository,branch:task.branch,requiredAncestors:[task.currentCommit]}),control=await verifyRemote({repository,branch:"stage13/control-plane-approved-delivery-runtime",requiredAncestors:[predecessor.runtimeVersion,runtimeVersion]});
  requireProof(product?.currentTip===task.currentCommit&&product.ancestors?.[task.currentCommit]===true,"live_product_tip");
  requireProof(control?.currentTip===runtimeVersion&&control.ancestors?.[runtimeVersion]===true&&control.ancestors?.[predecessor.runtimeVersion]===true,"runtime_transition");
  const dependencyPreflight={package:"@neondatabase/serverless",cwd:predecessor.workspaceRoot,required:true,consumesOnFailure:true};
  const approvalArguments={taskId,projectId:task.projectId,expectedVersion:task.stateVersion,repository,branch:task.branch,currentCommit:task.currentCommit,workspaceRoot:predecessor.workspaceRoot,runtimeVersion,planHash:plan.planHash,planGenerationId:predecessor.planGenerationId,fullPlanHash:recoveryHash(plan),filesHash:recoveryHash(plan.files),focusedTests:predecessor.focusedTests,focusedCounts,sourcePlanStepId:predecessor.sourcePlanStepId,sourceApplyStepId:apply.stepId,sourceFocusedStepId:focused.stepId,sourceApplyHash:recoveryHash(apply),sourceFocusedHash:recoveryHash(focused),sourceLineageHash:recoveryHash(apply.result.taskOwnedDirtyLineage),predecessorGenerationId:activeGeneration(predecessor),failedSourceStepId:failed.stepId,failedSourceStepHash:recoveryHash(failed),failedFailureFingerprint:diagnostics.fingerprint,failedCounts:diagnostics.counts,failedFullTestAuthorityHash:recoveryHash(predecessor),workspaceProofHash:recoveryHash(proof),workspaceEvidenceHash:recoveryHash(entries),workspaceScopeHash:recoveryHash(requiredPaths),dependencyPreflight,maxRecoveries:1,runtimeMinutes:FULL_TEST_SCOPE_RUNTIME_MINUTES,maxSteps:1,maxFullTestRuns:1,maxFullTestRetries:1,maxProductMutations:0,maxAdditionalAttempts:0};
  return{task,steps,...state,approvalArguments,proof};
}

export const recoverFullTestScope=options=>recoverFullTestScopeRecord(options,describeFullTestScopeRecovery,FULL_SCOPE);
export const recoverFailedFullTestRetry=options=>recoverFullTestScopeRecord(options,describeFailedFullTestRetry,RETRY_SCOPE);
async function recoverFullTestScopeRecord(options,describe,descriptor){
  const {input,storage,ownerId,clock=()=>new Date()}=options,state=await describe(options),{task,steps,plan,predecessor,apply,focused,entries,requiredPaths,approvalArguments}=state;
  const approval=typeof input.approvalId==="string"?await storage.getApproval(input.approvalId,ownerId):null;
  requireProof(approval&&approval.id===input.approvalId&&approval.ownerId===ownerId&&approval.projectId===task.projectId&&approval.status==="approved"&&approval.runId===task.id&&approval.tool===descriptor.tool&&same(approval.arguments,approvalArguments),"exact_owner_approval");
...
export function validateFullTestScopeEvidence(task,steps,clock=()=>new Date()){
  ...
  if(descriptor.isRetry){const failed=steps.find(step=>step.stepId===record.failedSourceStepId),previous=task.metadata.fullTestScopeRecoveryHistory?.[0];requireProof(record.maxFullTestRetries===1&&record.approvalArguments.maxFullTestRetries===1&&same(record.dependencyPreflight,{package:"@neondatabase/serverless",cwd:record.workspaceRoot,required:true,consumesOnFailure:true})&&same(record.dependencyPreflight,record.approvalArguments.dependencyPreflight)&&record.failedSourceStepId===record.approvalArguments.failedSourceStepId&&record.failedSourceStepHash===recoveryHash(failed)&&record.failedSourceStepHash===record.approvalArguments.failedSourceStepHash&&record.failedFailureFingerprint===failed?.result?.diagnostics?.fingerprint&&record.failedFailureFingerprint===record.approvalArguments.failedFailureFingerprint&&same(record.failedCounts,record.approvalArguments.failedCounts)&&previous?.consumed===true&&record.approvalArguments.failedFullTestAuthorityHash===recoveryHash(previous),"approved_failed_full_test_retry_binding");}
  ...
  if(descriptor.isRetry&&executions[0]?.status==="completed")requireProof(same(executions[0].result.dependencyPreflight,{package:"@neondatabase/serverless",cwd:record.workspaceRoot,resolved:true}),"completed_dependency_preflight");
  return{record,history,descriptor,plan,fullTest:executions[0]};
}
