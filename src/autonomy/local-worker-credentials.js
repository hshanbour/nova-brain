...
export function createWindowsCredentialStore({platform=process.platform,spawn=spawnChild,killTree=defaultKillTree,isAlive=defaultAlive,environment=process.env,helperTimeoutMs=60000}={}){
  ...
  const invoke=(action,name,secret,secondName)=>new Promise((resolve,reject)=>{
    const args=["-NoLogo","-NoProfile","-NonInteractive","-ExecutionPolicy","Bypass","-File",SCRIPT,action,TARGET(name)];if(secondName)args.push(TARGET(secondName));
    const startedAt=new Date().toISOString(),type=secondName?"credential_set":TYPE(name),child=spawn(executable,args,{windowsHide:true,stdio:[secret===undefined?"ignore":"pipe","pipe","pipe"]});
    ...
    child.stderr.on("data",chunk=>{stderrBytes+=chunk.length;const text=String(chunk);for(const match of text.matchAll(/NOVA_STAGE:([a-z_]+)/g)){const marker=match[1];if(["powershell_started","native_api_loaded","credential_api_read","credential_api_complete"].includes(marker))stage=marker;}for(const match of text.matchAll(/NOVA_ERROR:([a-z_]+)/g))safeError=match[1];});
