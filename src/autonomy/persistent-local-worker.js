...
export function createPersistentLocalWorker({client,root,branch="feat/nova-brain-mvp-foundation",repository="hshanbour/nova-brain",runtimeVersion,environment=process.env,workerId=`persistent-local-${randomUUID()}`,registry}={}){
  ...
  async function handoff(task){
    ...
    if(!job||job.taskId!==task.id||job.branch!==branch||job.expectedCommit!==task.expectedCommit||(!ALLOWED.has(job.tool)&&!approvedPush)||!exactFullTestScope(task,job,{repository,branch,root:controlledRoot,runtimeVersion,workerId})||!exactExecutionScope(task,job,{repository,branch,root:controlledRoot,runtimeVersion,workerId}))throw Object.assign(new Error("Server returned an invalid bounded handoff."),{code:"invalid_handoff"});
