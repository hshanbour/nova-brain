...
export async function runPersistentWorkerService({baseUrl,intervalMs=5000,version="local",probeOnly=false,maxIterations=Infinity,shouldStop=()=>false,delay=sleep,credentialLoader=loadLocalWorkerCredentials,clientFactory=createLocalWorkerClient,workerFactory=createPersistentLocalWorker,acquireInstance=acquirePersistentWorkerInstance,statusWriter=writePersistentWorkerStatus,credentialTimeoutMs=12000,authTimeoutMs=20000}={}){
  ...
  try{
    ...
    try{client=clientFactory({baseUrl,novaToken:credentials.novaToken,vercelBypassToken:credentials.vercelBypassToken,requestTimeoutMs:authTimeoutMs});worker=workerFactory({client});}catch(error){await report({state:"fatal",code:safeCode(error)});return{started:false,reason:safeCode(error)};}
