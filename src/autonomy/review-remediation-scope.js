...
const REMEDIATION_DESCRIPTORS=Object.freeze([OBSERVABLE_LINKAGE_DESCRIPTOR,SOURCE_LITERAL_DESCRIPTOR,IMPLEMENTATION_CONTENT_DESCRIPTOR,EVIDENCE_BOUND_DESCRIPTOR,SOURCE_BOUND_DESCRIPTOR,REJECTED_DESCRIPTOR,DESCRIPTOR]);
const predecessorDescriptor=descriptor=>descriptor===OBSERVABLE_LINKAGE_DESCRIPTOR?SOURCE_LITERAL_DESCRIPTOR:descriptor===SOURCE_LITERAL_DESCRIPTOR?IMPLEMENTATION_CONTENT_DESCRIPTOR:descriptor===IMPLEMENTATION_CONTENT_DESCRIPTOR?EVIDENCE_BOUND_DESCRIPTOR:descriptor===EVIDENCE_BOUND_DESCRIPTOR?SOURCE_BOUND_DESCRIPTOR:descriptor===SOURCE_BOUND_DESCRIPTOR?REJECTED_DESCRIPTOR:DESCRIPTOR;
...
function remediationSteps(record){
  ...
  return[...record.requiredPaths.map(path=>({type:"read_files",capability:"repo_read_remote",input:{tool:"repo_read_task_owned_local",arguments:{path,expectedContentHash:record.beforeEntries.find(item=>item.path===path).contentHash,binding}},idempotencyIdentity:`review-remediation:${record.taskId}:${record.fromStateVersion}:read:${path}`})),
    {type:"plan_repair",capability:"reasoning",input:{tool:"self_development_plan_implementation",arguments:{taskId:record.taskId,currentCommit:"$CURRENT_COMMIT",candidatePaths:record.requiredPaths,failureEvidence:{code:record.observableLinkageReplan?"review_coverage_observable_linkage_invalid":record.sourceLiteralReplan?"review_coverage_source_literal_invalid":record.implementationContentReplan?"implementation_content_invalid":"review_remediation_required",reviewHash:record.reviewHash,review:record.review,mutationApplied:false,...(record.implementationContentReplan?{requiredImplementationContentContract:{operation:"replace",contentType:"string",completeReplacement:true,minLength:1,maxLength:250000,instructionsOrPlaceholdersForbidden:true}}:{}),...(record.sourceLiteralReplan?{requiredReviewCoverageContract:{sourceExcerpt:"exact non-placeholder literal substring of the proposed replacement source for testPath",testName:"same named test declaration contained in sourceExcerpt",stimulus:"literal stimulus code contained in sourceExcerpt and preceding assertion",assertion:"literal behavioral assertion contained in sourceExcerpt",observableCodeReference:record.observableLinkageReplan?"real code reference present in the exact proposed replacement source and exercised by the same source-bound behavioral test":"required by the existing source-bound behavioral coverage validator",coherentBehavioralTest:"testName, sourceExcerpt, observableCodeReference, stimulus, and assertion must describe the same test behavior",staleOrPlaceholderEvidenceForbidden:true}}:{})}}}},
...
async function describeRejectedRemediation(options,descriptor,sourceDescriptor){
  ...
  let privateEvidence;
  if(sourceDescriptor===EVIDENCE_BOUND_DESCRIPTOR||sourceDescriptor===IMPLEMENTATION_CONTENT_DESCRIPTOR||sourceDescriptor===SOURCE_LITERAL_DESCRIPTOR){
