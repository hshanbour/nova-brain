...
export const FAILED_SEMANTIC_READ_RECOVERY_CLASS="owner_approved_failed_semantic_read_recovery";
export const FAILED_SEMANTIC_READ_RECOVERY_TOOL="self_development_failed_semantic_read_recovery";
export const TEST_IDENTITY_INVENTORY_REPLAN_CLASS="owner_approved_test_identity_inventory_review_replan";
export const TEST_IDENTITY_INVENTORY_REPLAN_TOOL="self_development_test_identity_inventory_review_replan";
...
const FAILED_SEMANTIC_READ_DESCRIPTOR=Object.freeze({historyKey:"failedSemanticReadRecoveryHistory",boundaryKey:"failedSemanticReadRecoveryBoundary",recoveryClass:FAILED_SEMANTIC_READ_RECOVERY_CLASS,tool:FAILED_SEMANTIC_READ_RECOVERY_TOOL,dependencyPreflight:true,implementationContentReplan:true,sourceLiteralReplan:true,observableLinkageReplan:true,semanticEvidenceReplan:true,failedReadRecovery:true});
const TEST_IDENTITY_INVENTORY_DESCRIPTOR=Object.freeze({historyKey:"testIdentityInventoryReviewReplanHistory",boundaryKey:"testIdentityInventoryReviewReplanBoundary",recoveryClass:TEST_IDENTITY_INVENTORY_REPLAN_CLASS,tool:TEST_IDENTITY_INVENTORY_REPLAN_TOOL,dependencyPreflight:true,implementationContentReplan:true,sourceLiteralReplan:true,observableLinkageReplan:true,semanticEvidenceReplan:true,testIdentityInventoryReplan:true});
const REMEDIATION_DESCRIPTORS=Object.freeze([TEST_IDENTITY_INVENTORY_DESCRIPTOR,FAILED_SEMANTIC_READ_DESCRIPTOR,SEMANTIC_EVIDENCE_DESCRIPTOR,OBSERVABLE_LINKAGE_DESCRIPTOR,SOURCE_LITERAL_DESCRIPTOR,IMPLEMENTATION_CONTENT_DESCRIPTOR,EVIDENCE_BOUND_DESCRIPTOR,SOURCE_BOUND_DESCRIPTOR,REJECTED_DESCRIPTOR,DESCRIPTOR]);
const predecessorDescriptor=descriptor=>descriptor===TEST_IDENTITY_INVENTORY_DESCRIPTOR?FAILED_SEMANTIC_READ_DESCRIPTOR:descriptor===FAILED_SEMANTIC_READ_DESCRIPTOR?SEMANTIC_EVIDENCE_DESCRIPTOR:descriptor===SEMANTIC_EVIDENCE_DESCRIPTOR?OBSERVABLE_LINKAGE_DESCRIPTOR:descriptor===OBSERVABLE_LINKAGE_DESCRIPTOR?SOURCE_LITERAL_DESCRIPTOR:descriptor===SOURCE_LITERAL_DESCRIPTOR?IMPLEMENTATION_CONTENT_DESCRIPTOR:descriptor===IMPLEMENTATION_CONTENT_DESCRIPTOR?EVIDENCE_BOUND_DESCRIPTOR:descriptor===EVIDENCE_BOUND_DESCRIPTOR?SOURCE_BOUND_DESCRIPTOR:descriptor===SOURCE_BOUND_DESCRIPTOR?REJECTED_DESCRIPTOR:DESCRIPTOR;
...
function rejectedReviewSource(task,steps,sourceDescriptor=DESCRIPTOR,{privateEvidence,attestedSourceProof}={}){
  ...
  const semanticRejection=sourceDescriptor===OBSERVABLE_LINKAGE_DESCRIPTOR?validateSemanticEvidenceRejection({task,failed,diagnostics,evidence,predecessor,privateEvidence,attestedSourceProof}):sourceDescriptor===FAILED_SEMANTIC_READ_DESCRIPTOR?validateTestIdentityInventoryRejection({task,failed,diagnostics,evidence,predecessor,privateEvidence,attestedSourceProof}):null;
...
function validateTestIdentityInventoryRejection({task,failed,diagnostics,evidence,predecessor,privateEvidence,attestedSourceProof}){
  const reference=failed.result?.rejectedReviewEvidence,envelope=privateEvidence?.envelope,proof=attestedSourceProof;
  requireProof(reference?.persisted===true&&typeof reference.id==="string"&&(!privateEvidence||privateEvidence.id===reference.id&&privateEvidence.ownerId===task.ownerId&&privateEvidence.taskId===task.id)&&(!proof||proof.privateEvidenceId===reference.id),"private_test_identity_inventory_evidence_reference");
  const coverage=diagnostics?.coverageDiagnostics,first=coverage?.constraints?.[0],constraint=predecessor.review?.acceptanceConstraints?.[0],predicate="semantic_test_identity_binding",subclause="test_identity_exists_in_exact_source";
  requireProof(diagnostics?.predicate===predicate&&diagnostics.mutationApplied===false&&same(coverage?.firstFailure,{predicate,constraintId:constraint?.id,subclause})&&first?.evaluated===true&&first.firstFailedSubclause===subclause&&same(first.clauses,[{predicate,subclause:"test_name_not_placeholder",passed:true},{predicate,subclause,passed:false}])&&HASH.test(first.expectedSourceHash||"")&&first.expectedSourceHash===first.suppliedSourceHash&&first.suppliedSourceHashPresent===true&&predecessor.requiredPaths.includes(first.testPath)&&evidence?.requestedFocusedTests?.some(item=>item.path===first.testPath&&item.kind==="existing")&&coverage.constraints.slice(1).every(item=>item.evaluated===false&&item.firstFailedSubclause===null&&same(item.clauses,[])),"exact_test_identity_inventory_rejection");
  if(envelope){
    const item=envelope.coverage?.find(entry=>entry.constraintId===constraint.id);
    requireProof(envelope.version===1&&envelope.diagnosticOnly===true&&envelope.executionAuthorized===false&&envelope.mutationApplied===false&&envelope.taskId===task.id&&envelope.stateVersion===task.stateVersion-1&&envelope.executionId===failed.stepId&&envelope.attempt===1&&envelope.continuationGenerationId===predecessor.activeContinuation.generationId&&envelope.rejectionPredicate===predicate&&envelope.planFingerprint===evidence.planFingerprint&&Array.isArray(envelope.proposedMutationPaths)&&envelope.proposedMutationPaths.length>0&&envelope.proposedMutationPaths.length<=8&&envelope.proposedMutationPaths.every(path=>predecessor.requiredPaths.includes(path))&&Array.isArray(envelope.selectedFocusedTestPaths)&&envelope.selectedFocusedTestPaths.includes(first.testPath)&&item?.testPath===first.testPath&&item.sourceOrigin==="fresh_read"&&item.expectedSourceHash===first.expectedSourceHash&&item.suppliedSourceHash===first.suppliedSourceHash&&text(item.testName,300)&&item.firstFailedSubclause===subclause,"private_test_identity_inventory_evidence");
    return{testPath:item.testPath,testName:item.testName,sourceHash:item.expectedSourceHash};
  }
  requireProof(HASH.test(proof?.privateEvidenceHash||"")&&HASH.test(proof?.rejectedPlanFingerprint||""),"attested_private_test_identity_inventory_evidence");
  return null;
}
...
export async function describeTestIdentityInventoryReviewReplan(options){
  return describeRejectedRemediation(options,TEST_IDENTITY_INVENTORY_DESCRIPTOR,FAILED_SEMANTIC_READ_DESCRIPTOR);
}

function failedSemanticReadSource(task,steps){
  const history=task.metadata?.[SEMANTIC_EVIDENCE_DESCRIPTOR.historyKey],predecessor=history?.[0],boundary=task.metadata?.[SEMANTIC_EVIDENCE_DESCRIPTOR.boundaryKey],active=predecessor?.activeContinuation,failed=steps.find(step=>step.stepId===predecessor?.readStepIds?.[0]);
  requireProof(history?.length===1&&predecessor?.recoveryClass===SEMANTIC_EVIDENCE_DESCRIPTOR.recoveryClass&&predecessor.authorizationConsumed===true&&predecessor.consumed===true&&predecessor.result==="failed"&&predecessor.planningClaimed===false&&!predecessor.planHash&&!predecessor.planGenerationId&&!predecessor.afterEntries,"consumed_failed_semantic_read_predecessor");
  requireProof(predecessor.workerBindingState==="bound"&&WORKER.test(predecessor.workerId||"")&&same(predecessor.claimedStepIds,[predecessor.readStepIds[0]]),"failed_read_worker_and_reservation");
  requireProof(task.status==="blocked"&&task.currentPhase==="read_files"&&task.currentStep===active.startStep&&task.errorCode==="path_traversal"&&task.repairIteration===3&&task.repairIteration===predecessor.repairIteration&&!task.leaseOwner&&!task.leaseToken&&!task.metadata.localHandoff&&!task.approvalState,"failed_semantic_read_boundary");
  requireProof(failed?.taskId===task.id&&failed.stepType==="read_files"&&failed.status==="failed"&&failed.attempt===1&&failed.errorCode==="path_traversal"&&failed.result?.code==="path_traversal"&&failed.result?.message==="Path is outside the approved repository."&&same(failed.input,task.metadata.steps[active.startStep]?.input)&&!steps.some(step=>ordinal(step)>ordinal(failed)),"exact_failed_semantic_read_execution");
  const operationFingerprint=recoveryHash([task.id,task.currentStep,failed.stepType,task.metadata.steps[active.startStep].input,task.currentCommit,active.generationId]);
  requireProof(failed.operationFingerprint===operationFingerprint,"failed_read_operation_fingerprint");
  const expectedBoundary={kind:"product_repair_decision",executionAuthorized:false,reviewHash:predecessor.reviewHash,planHash:null,planGenerationId:null,stepId:failed.stepId,errorCode:"path_traversal",findingsResolved:false,mutationApplied:null};
  requireProof(same(boundary,expectedBoundary)&&same(predecessor.boundary,boundary)&&same(task.metadata.activeContinuation,active),"failed_read_decision_boundary");
  const started=Date.parse(failed.startedAt),completed=Date.parse(failed.completedAt),stopped=Date.parse(predecessor.completedAt);
  requireProof(Number.isFinite(started)&&completed>=started&&stopped>=started&&started>=Date.parse(active.runtimeStartedAt)&&completed<=Date.parse(active.runtimeDeadline)&&stopped<=Date.parse(active.runtimeDeadline),"historical_failed_read_runtime");
  const projected=structuredClone(task),projectedRecord=projected.metadata[SEMANTIC_EVIDENCE_DESCRIPTOR.historyKey][0];
  projected.status="running";projected.currentStep=active.startStep;projected.currentPhase="read_files";projected.errorCode=null;projected.blockedReason=null;
  projectedRecord.consumed=false;delete projectedRecord.completedAt;delete projectedRecord.result;delete projectedRecord.boundary;delete projected.metadata[SEMANTIC_EVIDENCE_DESCRIPTOR.boundaryKey];
  validateReviewRemediationEvidence(projected,steps.filter(step=>step!==failed),()=>new Date(started));
  const sourceProof={version:1,sourceVersion:task.stateVersion,failedStepId:failed.stepId,failedStepHash:recoveryHash(failed),predecessorGenerationId:active.generationId,predecessorHash:recoveryHash(predecessor),boundaryHash:recoveryHash(boundary)};
  return{predecessor,failed,sourceProof};
}

export async function describeFailedSemanticReadRecovery(options){
  const{taskId=options.task?.id,input,actor,storage,ownerId,repository,approvedBranch,runtimeVersion,verifyRemote}=options,task=options.task||await options.runtime.get(taskId),steps=options.steps||await options.runtime.steps(taskId);
  requireProof(keys(input,["expectedVersion","planHash","runtimeVersion","workspaceProof","workspaceProofSignature","review","approvalId"])&&Number.isInteger(input.expectedVersion)&&HASH.test(input.planHash||"")&&SHA.test(runtimeVersion||"")&&input.runtimeVersion===runtimeVersion,"request_identity");
  const proof=input.workspaceProof;
  requireProof(keys(proof,["taskId","expectedVersion","runtimeVersion","workspace"])&&actor?.actorType==="scoped_local_worker"&&same(actor.workspaceProof,proof)&&proof.taskId===taskId&&proof.expectedVersion===input.expectedVersion&&proof.runtimeVersion===runtimeVersion,"authenticated_workspace_proof");
  requireProof(task?.id===taskId&&task.taskType==="self_development"&&task.stateVersion===input.expectedVersion&&!task.metadata?.[FAILED_SEMANTIC_READ_DESCRIPTOR.historyKey]?.length&&task.branch===approvedBranch&&task.metadata?.selfDevelopment?.repository===repository&&SHA.test(task.currentCommit||"")&&task.currentCommit===task.startingCommit,"single_use_failed_read_identity");
  const{predecessor,failed,sourceProof}=failedSemanticReadSource(task,steps),oldApproval=await storage.getApproval(predecessor.approvalId,ownerId);
  requireProof(oldApproval?.ownerId===ownerId&&oldApproval.projectId===task.projectId&&oldApproval.runId===task.id&&oldApproval.status==="approved"&&oldApproval.tool===SEMANTIC_EVIDENCE_DESCRIPTOR.tool&&same(oldApproval.arguments,predecessor.approvalArguments),"consumed_semantic_review_approval");
  ...
  requireProof(input.planHash===predecessor.sourcePlanHash&&same(input.review,review)&&same(sourcePlan,predecessor.sourcePlanSnapshot),"immutable_failed_read_review_binding");
  const workspace=proof.workspace;
  requireProof(keys(workspace,["root","gitTopLevel","repository","branch","head","liveTip","clean","changedFiles"])&&recoveryRoot(workspace.root)===predecessor.workspaceRoot&&recoveryRoot(workspace.gitTopLevel)===predecessor.workspaceRoot&&workspace.repository===repository&&workspace.branch===task.branch&&workspace.head===task.currentCommit&&workspace.liveTip===task.currentCommit&&workspace.clean===false&&Array.isArray(workspace.changedFiles)&&workspace.changedFiles.length===8&&new Set(workspace.changedFiles.map(item=>item?.path)).size===8&&same(proofEntries(workspace.changedFiles),proofEntries(entries)),"current_eight_file_workspace");
  requireProof(typeof verifyRemote==="function","remote_verifier");
  const product=await verifyRemote({repository,branch:task.branch,requiredAncestors:[task.currentCommit]}),control=await verifyRemote({repository,branch:"stage13/control-plane-approved-delivery-runtime",requiredAncestors:[predecessor.runtimeVersion,runtimeVersion]});
...
async function describeRejectedRemediation(options,descriptor,sourceDescriptor){
  ...
  let privateEvidence;
  if(sourceDescriptor===EVIDENCE_BOUND_DESCRIPTOR||sourceDescriptor===IMPLEMENTATION_CONTENT_DESCRIPTOR||sourceDescriptor===SOURCE_LITERAL_DESCRIPTOR||sourceDescriptor===OBSERVABLE_LINKAGE_DESCRIPTOR||sourceDescriptor===FAILED_SEMANTIC_READ_DESCRIPTOR){
...
export async function recoverFailedSemanticReadRecovery(options){
  return recoverBoundRemediation(options,await describeFailedSemanticReadRecovery(options),FAILED_SEMANTIC_READ_DESCRIPTOR);
}

export async function recoverTestIdentityInventoryReviewReplan(options){
  return recoverBoundRemediation(options,await describeTestIdentityInventoryReviewReplan(options),TEST_IDENTITY_INVENTORY_DESCRIPTOR);
}
...
function baseEvidence(task,steps,clock){
  ...
  if(descriptor===FAILED_SEMANTIC_READ_DESCRIPTOR){
    requireProof(same(record.rejectedPlanSource,record.approvalArguments.rejectedPlanSource)&&record.reviewSourceVersion===(predecessor.reviewSourceVersion??predecessor.fromStateVersion)&&record.reviewSourceVersion===record.approvalArguments.reviewSourceVersion&&record.rejectedPlanSource?.sourceVersion===record.fromStateVersion,"approved_failed_read_source_binding");
    const projected=structuredClone(task);
    projected.status="blocked";projected.stateVersion=record.fromStateVersion;projected.currentStep=predecessor.activeContinuation.startStep;projected.currentPhase="read_files";projected.errorCode="path_traversal";
    ...
    const historical=failedSemanticReadSource(projected,steps.filter(step=>ordinal(step)<=active.startStep));
    requireProof(same(historical.sourceProof,record.rejectedPlanSource),"immutable_failed_read_source_proof");
