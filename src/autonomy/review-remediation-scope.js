...
export const reviewRemediationDescriptor=task=>task?.metadata?.reviewRemediationHistory!==undefined||task?.metadata?.activeContinuation?.recoveryClass===REVIEW_REMEDIATION_CLASS?DESCRIPTOR:null;
...
function immutableHistories(task){const fixed=["selfDevelopment","maxRepairIterations"],historical=Object.keys(task.metadata||{}).filter(key=>/(?:History|Boundary)$/.test(key)&&!["continuationHistory",DESCRIPTOR.historyKey,DESCRIPTOR.boundaryKey].includes(key));return Object.fromEntries([...new Set([...fixed,...historical])].sort().map(key=>[key,recoveryHash(task.metadata[key])]));}
...
export async function recoverReviewRemediation(options){
  const state=await describeReviewRemediation(options),{task,steps,predecessor,sourceApply,sourceFocused,sourceFull,sourcePlan,entries,requiredPaths,review,reviewHash,approvalArguments}=state,{input,storage,ownerId,clock=()=>new Date()}=options;
  const approval=input.approvalId?await storage.getApproval(input.approvalId,ownerId):null;
  requireProof(approval?.id===input.approvalId&&approval.ownerId===ownerId&&approval.projectId===task.projectId&&approval.runId===task.id&&approval.status==="approved"&&approval.tool===REVIEW_REMEDIATION_TOOL&&same(approval.arguments,approvalArguments),"exact_owner_approval");
  const base=task.metadata.steps.length,now=clock().toISOString(),activeContinuation=createActiveContinuation({task,startStep:base,plannedSteps:13,repairLimit:0,recoveryClass:REVIEW_REMEDIATION_CLASS,runtimeStartedAt:now,runtimeMinutes:15});
...
function baseEvidence(task,steps,clock){
  const history=task.metadata?.reviewRemediationHistory,record=history?.[0],active=task.metadata?.activeContinuation;
  requireProof(history?.length===1&&record?.recoveryClass===REVIEW_REMEDIATION_CLASS&&record.authorizationConsumed===true&&record.consumed===false&&record.taskId===task.id&&record.repository===task.metadata.selfDevelopment?.repository&&record.branch===task.branch&&record.currentCommit===task.currentCommit&&task.currentCommit===task.startingCommit&&same(record.activeContinuation,active)&&same(active,task.metadata.continuationHistory?.at(-1))&&active.recoveryClass===REVIEW_REMEDIATION_CLASS&&active.maxSteps===13&&active.runtimeMinutes===15&&task.currentStep>=active.startStep&&task.currentStep<active.startStep+13&&["queued","waiting_for_worker","running","planning"].includes(task.status),"active_remediation_generation");
  ...
  return{record,history,active,executions};
}
