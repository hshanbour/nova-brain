...
const REMEDIATION_DESCRIPTORS=Object.freeze([IMPLEMENTATION_CONTENT_DESCRIPTOR,EVIDENCE_BOUND_DESCRIPTOR,SOURCE_BOUND_DESCRIPTOR,REJECTED_DESCRIPTOR,DESCRIPTOR]);
const predecessorDescriptor=descriptor=>descriptor===IMPLEMENTATION_CONTENT_DESCRIPTOR?EVIDENCE_BOUND_DESCRIPTOR:descriptor===EVIDENCE_BOUND_DESCRIPTOR?SOURCE_BOUND_DESCRIPTOR:descriptor===SOURCE_BOUND_DESCRIPTOR?REJECTED_DESCRIPTOR:DESCRIPTOR;
...
function remediationSteps(record){
  ...
  return[...record.requiredPaths.map(path=>({type:"read_files",capability:"repo_read_remote",input:{tool:"repo_read_task_owned_local",arguments:{path,expectedContentHash:record.beforeEntries.find(item=>item.path===path).contentHash,binding}},idempotencyIdentity:`review-remediation:${record.taskId}:${record.fromStateVersion}:read:${path}`})),
    {type:"plan_repair",capability:"reasoning",input:{tool:"self_development_plan_implementation",arguments:{taskId:record.taskId,currentCommit:"$CURRENT_COMMIT",candidatePaths:record.requiredPaths,failureEvidence:{code:record.implementationContentReplan?"implementation_content_invalid":"review_remediation_required",reviewHash:record.reviewHash,review:record.review,mutationApplied:false,...(record.implementationContentReplan?{requiredImplementationContentContract:{operation:"replace",contentType:"string",completeReplacement:true,minLength:1,maxLength:250000,instructionsOrPlaceholdersForbidden:true}}:{})}}}},
...
function rejectedReviewSource(task,steps,sourceDescriptor=DESCRIPTOR,{privateEvidence,attestedSourceProof}={}){
  ...
  const contentRejection=sourceDescriptor===EVIDENCE_BOUND_DESCRIPTOR,rejectionPredicate=sourceDescriptor===DESCRIPTOR?"complete_behavioral_coverage":contentRejection?"implementation_plan_invalid":"source_bound_behavioral_coverage",terminalError=contentRejection?"implementation_plan_invalid":"review_remediation_precondition_failed";
  ...
  const privateEnvelope=privateEvidence?.envelope,sourceProof={version:1,sourceVersion:task.stateVersion,failedStepId:failed.stepId,failedStepHash:recoveryHash(failed),rejectedPlanFingerprint:contentRejection?(privateEnvelope?.planFingerprint||attestedSourceProof?.rejectedPlanFingerprint):evidence.planFingerprint,predecessorGenerationId:predecessor.activeContinuation.generationId,predecessorHash:recoveryHash(predecessor),boundaryHash:recoveryHash(boundary),readEvidenceHash,...(contentRejection?{privateEvidenceId:failed.result.rejectedReviewEvidence.id,privateEvidenceHash:privateEnvelope?recoveryHash(privateEnvelope):attestedSourceProof?.privateEvidenceHash}: {})};
  requireProof(!contentRejection||HASH.test(sourceProof.rejectedPlanFingerprint||"")&&HASH.test(sourceProof.privateEvidenceHash||""),"implementation_content_source_proof");
...
async function describeRejectedRemediation(options,descriptor,sourceDescriptor){
  ...
  let privateEvidence;
  if(sourceDescriptor===EVIDENCE_BOUND_DESCRIPTOR){
