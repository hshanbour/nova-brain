...
export function createWorkerRuntime({
  ...
  async function completeNoChangePlanning(task, step, result) {
    ...
    const entries=Array.isArray(candidate?.evidenceEntries)?candidate.evidenceEntries:[],request=task.metadata?.selfDevelopment||{},explicitPaths=[...new Set([...(request.scope?.paths||[]),...(request.scope?.focusedTests||[])])].sort(),plannerPaths=Array.isArray(step.input?.arguments?.candidatePaths)?[...new Set(step.input.arguments.candidatePaths)].sort():[],automaticReplan=(task.metadata?.discoveryOnlyReplanHistory||[]).findLast(record=>record?.scopeSource==="automatic_durable_discovery"&&Array.isArray(record.candidatePaths)&&JSON.stringify([...new Set(record.candidatePaths)].sort())===JSON.stringify(plannerPaths)),expectedPaths=explicitPaths.length?explicitPaths:(automaticReplan?[...new Set(automaticReplan.candidatePaths)].sort():[]),mappedCriteria=new Set((candidate?.acceptanceMapping||[]).map(item=>item.criterion)),criteria=[...new Set(request.acceptanceCriteria||[])],proof=Array.isArray(candidate?.satisfactionEvidence)?candidate.satisfactionEvidence:[],proofCriteria=new Set(proof.map(item=>item.criterion));
    const valid=candidate?.version===1&&candidate.taskId===task.id&&candidate.currentCommit===task.currentCommit&&candidate.intent==="implementation"&&candidate.satisfactionStatus==="already_satisfied"&&Array.isArray(candidate.unresolvedPrerequisites)&&candidate.unresolvedPrerequisites.length===0&&candidate.goalHash===lifecycleHash(JSON.stringify(request.userGoal))&&candidate.decisionHash===lifecycleHash({...candidate,decisionHash:undefined})&&entries.length>=2&&JSON.stringify(entries.map(item=>item.path).sort())===JSON.stringify(expectedPaths)&&entries.some(item=>!item.path.startsWith("test/"))&&entries.some(item=>item.path.startsWith("test/"))&&entries.every(item=>reads.get(item.path)?.stepId===item.readStepId&&canonicalContentHash(reads.get(item.path)?.content)===item.contentHash)&&Array.isArray(candidate.focusedTests)&&candidate.focusedTests.length>0&&candidate.focusedTests.every(test=>test.kind==="existing"&&entries.some(item=>item.path===test.path))&&Array.isArray(candidate.acceptanceMapping)&&candidate.acceptanceMapping.length>0&&criteria.every(criterion=>mappedCriteria.has(criterion)&&proofCriteria.has(criterion))&&proof.every(item=>criteria.includes(item.criterion)&&item.criterionHash===lifecycleHash(item.criterion)&&entries.some(entry=>entry.path===item.path)&&typeof item.excerpt==="string"&&item.excerpt.length>0&&item.excerptHash===lifecycleHash(item.excerpt)&&reads.get(item.path)?.content.includes(item.excerpt)&&(candidate.acceptanceMapping.find(mapping=>mapping.criterion===item.criterion)?.files||[]).concat(candidate.acceptanceMapping.find(mapping=>mapping.criterion===item.criterion)?.tests||[]).includes(item.path));
  ...
  async function completePlanningBlocked(task,step,result){
    const blocked=result?.planningBlocked,valid=blocked?.version===1&&blocked.taskId===task.id&&blocked.currentCommit===task.currentCommit&&blocked.intent==="implementation"&&blocked.code==="implementation_prerequisite_unresolved"&&blocked.goalHash===lifecycleHash(JSON.stringify(task.metadata?.selfDevelopment?.userGoal))&&blocked.decisionHash===lifecycleHash({...blocked,decisionHash:undefined})&&Array.isArray(blocked.unresolvedPrerequisiteFingerprints)&&blocked.unresolvedPrerequisiteFingerprints.length>0;
    if(!valid)throw new WorkerError("implementation_evidence_incomplete","The blocked planning result is not bound to this exact task and objective.",{retryable:false});
    const safe={ok:true,planningBlocked:blocked};
    await storage.updateAutonomyStep(task.id,step.stepId,{status:"completed",result:redact(safe),completedAt:iso(clock)});
    const updated=await storage.updateAutonomyTask(task.id,ownerId,{status:"blocked",currentStep:task.currentStep+1,currentPhase:"plan_implementation",nextRunAt:null,errorCode:blocked.code,blockedReason:"Implementation planning stopped because a required contract, dependency, or evidence remains unresolved.",checkpoint:{...task.checkpoint,completedSteps:[...(task.checkpoint?.completedSteps||[]),step.stepId],pendingStep:null,latestResult:redact(safe)},metadata:{...task.metadata,requiredCapability:null},leaseOwner:null,leaseToken:null,leaseExpiresAt:null},task.stateVersion);
    if(!updated)throw new WorkerError("version_conflict","Task changed before the blocked planning boundary was persisted.",{retryable:false});
    await activity(updated,"self_development_planning_blocked","blocked","Implementation planning stopped at an unresolved prerequisite boundary.",{stepId:step.stepId,errorCode:blocked.code,decisionHash:blocked.decisionHash});
  }
