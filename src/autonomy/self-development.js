...
export function createSelfDevelopmentService({
  ...
  async function attestHandsWorkspace(taskId,input,actor={}){
    ...
    const legacyPaths=Array.isArray(apply?.result?.files)?[...new Set(apply.result.files.map(safePath))].sort():[],byPath=new Map((files||[]).map(file=>[safePath(file.path),file])),legacyExpected=legacyPaths.map(path=>{const file=byPath.get(path);return typeof file?.content==="string"?{path,hashAlgorithm:"git_sha1",hash:gitBlobHash(file.content)}:null;}),lineageExpected=testFailedEligible?[...durableLineage.entries].map(item=>({path:safePath(item?.path),contentHash:String(item?.contentHash||"").toLowerCase()})).sort((a,b)=>a.path.localeCompare(b.path)):[],actual=(workspace.changedFiles||[]).map(item=>({path:safePath(item?.path),hashAlgorithm:item?.hashAlgorithm,hash:String(item?.hash||"").toLowerCase(),...(item?.contentHash?{contentHash:String(item.contentHash).toLowerCase()}:{})})).sort((a,b)=>a.path.localeCompare(b.path)),dirtyExact=testFailedEligible?lineageExpected.length>0&&lineageExpected.length<=12&&lineageExpected.every(item=>REVIEW_HASH.test(item.contentHash))&&actual.length===lineageExpected.length&&actual.every((item,index)=>item.path===lineageExpected[index].path&&item.hashAlgorithm==="git_sha1"&&SHA.test(item.hash)&&item.contentHash===lineageExpected[index].contentHash):legacyPaths.length>0&&legacyPaths.length<=12&&!legacyExpected.some(item=>!item)&&actual.length===legacyExpected.length&&actual.every((item,index)=>item.path===legacyExpected[index].path&&item.hashAlgorithm==="git_sha1"&&item.hash===legacyExpected[index].hash);
