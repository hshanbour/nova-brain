...
export function createSelfDevelopmentService({
  ...
  async function recoverTestRunnerInfrastructure(taskId,input){
    ...
    const steps=await runtime.steps(current.id),failed=steps.filter(step=>step.stepType==="run_full_tests"&&step.status==="failed"&&step.errorCode==="test_failed").at(-1),evidence=failed?.result?.diagnostics,failedOrdinal=stepOrdinal(failed),isUnavailable=value=>value?.identity?.command==="npm:test"&&value?.durationMs===0&&value?.exitCode===1&&value?.stderrExcerpt==="spawn npm ENOENT"&&Array.isArray(value.failedFiles)&&value.failedFiles.length===0&&Array.isArray(value.failedTitles)&&value.failedTitles.length===0&&value?.counts&&Object.values(value.counts).every(item=>item===null),history=current.metadata?.fullTestRepairHistory||[],invalidHistory=history.filter(item=>item.fingerprint===evidence?.fingerprint&&steps.some(step=>step.stepId===item.failedStepId&&isUnavailable(step.result?.diagnostics))),plan=current.metadata?.selfDevelopmentImplementationPlan,focused=steps.some(step=>step.stepType==="run_focused_tests"&&step.status==="completed"&&stepOrdinal(step)===failedOrdinal-1),delivered=steps.some(step=>["commit","review_commit","push","deploy_preview"].includes(step.stepType)&&step.status==="completed"&&stepOrdinal(step)>failedOrdinal);
    if(current.status!=="failed"||current.errorCode!=="repair_limit_reached"||!failed||failedOrdinal!==current.currentStep+1||!isUnavailable(evidence)||!invalidHistory.length||!focused||delivered||current.approvalState||current.leaseOwner||!plan?.provenance||current.metadata?.activeImplementationPlanGeneration!==plan.provenance.generationId||plan.provenance.taskId!==current.id||plan.provenance.currentCommit!==current.currentCommit)throw new SelfDevelopmentError("test_runner_recovery_precondition_failed","Only the exact false repair-limit caused by unavailable npm execution may be recovered.");
    assertActiveImplementationPlan(current,plan.files);
    const invalidIds=new Set(invalidHistory.map(item=>item.failedStepId)),remaining=current.metadata.steps.slice(failedOrdinal),base=current.metadata.steps.length,recoverySteps=[{type:"run_full_tests",input:{tool:"test_run_full",arguments:{}},idempotencyIdentity:`test-runner-infrastructure-recovery:${evidence.fingerprint}`},...remaining];
    ...
    const now=clock().toISOString(),activeContinuation=createActiveContinuation({task:current,startStep:base,plannedSteps:recoverySteps.length,repairLimit:Math.min(2,current.metadata?.selfDevelopment?.repairLimit??2),recoveryClass:"test_runner_unavailable_reclassification",runtimeStartedAt:now,runtimeMinutes:15}),record={recoveryClass:"test_runner_unavailable_reclassification",fromStateVersion:current.stateVersion,failedStepId:failed.stepId,fingerprint:evidence.fingerprint,retiredRepairHistoryStepIds:[...invalidIds],recoveredAt:now},metadata={...current.metadata,steps:[...current.metadata.steps,...recoverySteps],requiredCapability:"test_local",autoDispatch:true,fullTestRepairHistory:history.filter(item=>!invalidIds.has(item.failedStepId)),activeContinuation,continuationHistory:[...(current.metadata?.continuationHistory||[]),activeContinuation],testRunnerInfrastructureRecoveryHistory:[...(current.metadata?.testRunnerInfrastructureRecoveryHistory||[]),record]};
