...
export function createSelfDevelopmentService({
  ...
  async function recoverImplementationSchema(taskId, input) {
    ...
    const steps = await runtime.steps(current.id), semantic = resolveSemanticPlanApplyState(current, steps, "schema_mismatch"), {failed, planned, planTemplate, remaining, completedAfterPlan: unsafeAfterPlan} = semantic, approvals = await storage.listApprovals(ownerId, { limit: 100 }), schemaMessage = String(failed?.result?.message || "");
    if (current.status !== "failed" || current.errorCode !== "schema_mismatch" || !schemaMessage.includes("repo_apply_patch.currentCommit") || remaining[0]?.type !== "apply_patch" || unsafeAfterPlan || approvals.some((approval) => approval.runId === current.id) || current.metadata?.lastDeploymentId || current.metadata?.selfDevelopmentDeliveryAttestation)
  ...
  async function recoverHandsCommitMismatch(taskId,input,{failureCode="commit_mismatch",recoveryClass="commit_mismatch_descendant_rebind"}={}){
    ...
    const steps=await runtime.steps(current.id),semantic=resolveSemanticPlanApplyState(current,steps,failureCode),{failed,planned,planTemplate,remaining,completedAfterPlan}=semantic,approvals=await storage.listApprovals(ownerId,{limit:100}),oldCommit=current.currentCommit,newCommit=currentCommit;
