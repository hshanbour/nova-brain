...
export function createSelfDevelopmentService({
  ...
  async function attestHandsWorkspace(taskId,input,actor={}){
    ...
    const attestations=(await storage.listActivity(ownerId,{runId:current.id,limit:100})).filter(item=>item.action==="self_development_workspace_attested"),latest=attestations[0],value=latest?.metadata,exactPrior=latest?.status==="completed"&&value?.attestationVersion===1&&value.representation==="current_verified_workspace_linked_to_historical_apply"&&value.historicalProvenanceClaim===false&&value.taskId===current.id&&value.taskStateVersion===current.stateVersion&&value.repository===repository&&value.branch===current.branch&&canonicalWorkspaceRoot(value.workspaceRoot)===canonicalWorkspaceRoot(workspace.root)&&value.productHead===current.currentCommit&&value.liveBranchTip===workspace.liveTip&&value.runtimeVersion===input.runtimeVersion&&value.workerId===input.workerId.trim()&&value.sourceApplyStepId===apply.stepId&&value.sourcePlanStepId===plan.stepId&&value.sourceApplyFingerprint===apply.operationFingerprint&&exactDirtyEvidence(value.dirtyFiles,actual);if(exactPrior)return{attestation:value,idempotent:true};
    const attestation={attestationId:randomUUID(),attestationVersion:1,representation:"current_verified_workspace_linked_to_historical_apply",historicalProvenanceClaim:false,taskId:current.id,taskStateVersion:current.stateVersion,repository,branch:current.branch,workspaceRoot:canonicalWorkspaceRoot(workspace.root),productHead:current.currentCommit,liveBranchTip:workspace.liveTip,runtimeVersion:input.runtimeVersion,workerId:input.workerId.trim(),sourceApplyStepId:apply.stepId,sourcePlanStepId:plan.stepId,sourceApplyFingerprint:apply.operationFingerprint,dirtyFiles:actual,supersedesAttestationId:value?.attestationId||null,attestedAt:clock().toISOString()};
