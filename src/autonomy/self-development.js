...
export function createSelfDevelopmentService({
  ...
  async function recoverHandsCommitMismatch(taskId,input,{failureCode="commit_mismatch",recoveryClass="commit_mismatch_descendant_rebind"}={}){
    ...
    const implementation=planned.result.implementationPlan,hasBoundPlan=Boolean(implementation?.provenance);if(hasBoundPlan)assertActiveImplementationPlan(current,implementation.files);const declaredDirty=Array.isArray(input.workspace.changedFiles)?input.workspace.changedFiles:[],replaceOnly=implementation.files.every(file=>file.operation==="replace"),expectedDirty=implementation.files.map(file=>({path:safePath(file.path),hashAlgorithm:"git_sha1",hash:gitBlobHash(file.expectedContent)})).sort((a,b)=>a.path.localeCompare(b.path)),actualDirty=declaredDirty.map(item=>({path:safePath(item?.path),hashAlgorithm:item?.hashAlgorithm,hash:String(item?.hash||"").toLowerCase()})).sort((a,b)=>a.path.localeCompare(b.path)),taskOwnedDirty=input.workspace.clean===false&&failureCode==="commit_mismatch"&&hasBoundPlan&&replaceOnly&&actualDirty.length===expectedDirty.length&&actualDirty.every((item,index)=>item.path===expectedDirty[index].path&&item.hashAlgorithm==="git_sha1"&&item.hash===expectedDirty[index].hash);if(input.workspace.clean===true&&declaredDirty.length||input.workspace.clean===false&&!taskOwnedDirty)throw new SelfDevelopmentError("hands_context_recovery_dirty_unproven","A dirty checkout is allowed only when every changed file exactly matches the active plan's pre-mutation content.",400);
    ...
    const reboundTask={...current,currentCommit:newCommit},reboundPlan=changedPaths.length?null:hasBoundPlan?rebindEquivalentImplementationPlan({task:reboundTask,plan:implementation,evidence:implementation.files.filter(file=>file.operation==="replace").map(file=>({path:file.path,content:file.expectedContent}))}):implementation,reboundMetadata=hasBoundPlan&&reboundPlan?planLifecycleMetadata(reboundTask,reboundPlan):current.metadata,now=clock().toISOString(),record={recoveryClass,previousStateVersion:current.stateVersion,failedStepId:failed.stepId,plannedStepId:planned.stepId,semanticPlanOrdinal:semantic.plannedOrdinal,semanticApplyOrdinal:semantic.failedOrdinal,previousCurrentCommit:oldCommit,newCurrentCommit:newCommit,ancestryVerified:true,evidence:Object.fromEntries(evidencePaths.map(path=>[path,{oldBlob:evidence[path].oldSha,newBlob:evidence[path].newSha,equivalent:evidence[path].equivalent===true}])),invalidatedEvidencePaths:changedPaths,repositoryRoot:input.workspace.root,gitTopLevel:input.workspace.gitTopLevel,workingTreeClean:input.workspace.clean,taskOwnedDirtyFiles:taskOwnedDirty?actualDirty:[],previousPlanGenerationId:implementation.provenance?.generationId||null,newPlanGenerationId:reboundPlan?.provenance?.generationId||null,recoveredAt:now},requiredCapability=changedPaths.length?"repo_read_remote":"repo_mutate_local",status=changedPaths.length?"queued":"waiting_for_worker";
    const updated=await storage.updateAutonomyTask(current.id,ownerId,{status,currentStep:base,currentPhase:"hands_repository_context_recovery",currentCommit:newCommit,maxSteps,nextRunAt:now,startedAt:now,completedAt:null,errorCode:null,retryCount:0,blockedReason:null,checkpoint:{...current.checkpoint,pendingStep:null},metadata:{...reboundMetadata,steps:[...current.metadata.steps,...nextSteps],requiredCapability,autoDispatch:true,selfDevelopmentImplementationPlan:changedPaths.length?null:reboundPlan,activeImplementationPlanGeneration:changedPaths.length?null:reboundPlan?.provenance?.generationId||current.metadata.activeImplementationPlanGeneration,handsContextRecoveryHistory:[...(current.metadata.handsContextRecoveryHistory||[]),record],baseRevisionHistory:[...(current.metadata.baseRevisionHistory||[]),{previousBaseCommit:oldCommit,newBaseCommit:newCommit,previousStateVersion:current.stateVersion,ancestryVerified:true,recoveredAt:now}] }},current.stateVersion);if(!updated)throw new SelfDevelopmentError("version_conflict","Task changed during Hands-context recovery.");
    await storage.appendActivity({ownerId,projectId:current.projectId,runId:current.id,action:"self_development_hands_context_recovered",status,summary:"Exact pre-mutation Hands repository context and descendant task base were verified for bounded continuation.",metadata:{taskId:current.id,previousStateVersion:current.stateVersion,failedStepId:failed.stepId,previousCurrentCommit:oldCommit,newCurrentCommit:newCommit,ancestryVerified:true,invalidatedEvidencePaths:changedPaths,repositoryRoot:input.workspace.root,maxSteps}});
