...
export function createSelfDevelopmentService({
  ...
  async function attestHandsWorkspace(taskId,input,actor={}){
    ...
    if(current.status!=="failed"||current.errorCode!=="working_tree_dirty"||current.currentCommit!==workspace.head||current.branch!==workspace.branch||current.metadata?.selfDevelopment?.repository!==workspace.repository||current.leaseOwner||current.leaseToken)throw new SelfDevelopmentError("workspace_attestation_precondition_failed","Only the exact failed pre-mutation repair state may be attested.");
    ...
    const steps=await runtime.steps(current.id),apply=steps.find(step=>step.stepId===input.sourceApplyStepId&&step.stepType==="apply_patch"&&step.status==="completed"),applyOrdinal=stepOrdinal(apply),plan=steps.filter(step=>["plan_implementation","plan_repair"].includes(step.stepType)&&step.status==="completed"&&stepOrdinal(step)<applyOrdinal).at(-1),files=plan?.result?.implementationPlan?.files,paths=Array.isArray(apply?.result?.files)?[...new Set(apply.result.files.map(safePath))].sort():[],byPath=new Map((files||[]).map(file=>[safePath(file.path),file])),expected=paths.map(path=>{const file=byPath.get(path);return typeof file?.content==="string"?{path,hashAlgorithm:"git_sha1",hash:gitBlobHash(file.content)}:null;}),actual=(workspace.changedFiles||[]).map(item=>({path:safePath(item?.path),hashAlgorithm:item?.hashAlgorithm,hash:String(item?.hash||"").toLowerCase()})).sort((a,b)=>a.path.localeCompare(b.path));
    if(!apply||!plan||paths.length===0||paths.length>12||expected.some(item=>!item)||actual.length!==expected.length||actual.some((item,index)=>item.path!==expected[index].path||item.hashAlgorithm!=="git_sha1"||item.hash!==expected[index].hash))throw new SelfDevelopmentError("workspace_attestation_dirty_lineage_mismatch","Current dirty bytes do not exactly match the completed same-task apply lineage.");
    const prior=(await storage.listActivity(ownerId,{runId:current.id,limit:100})).find(item=>item.action==="self_development_workspace_attested"&&item.metadata?.taskStateVersion===current.stateVersion&&item.metadata?.sourceApplyStepId===apply.stepId);if(prior)return{attestation:prior.metadata,idempotent:true};
    const attestation={attestationId:randomUUID(),attestationVersion:1,representation:"current_verified_workspace_linked_to_historical_apply",historicalProvenanceClaim:false,taskId:current.id,taskStateVersion:current.stateVersion,repository,branch:current.branch,workspaceRoot:workspace.root,productHead:current.currentCommit,liveBranchTip:workspace.liveTip,runtimeVersion:input.runtimeVersion,workerId:input.workerId,sourceApplyStepId:apply.stepId,sourcePlanStepId:plan.stepId,sourceApplyFingerprint:apply.operationFingerprint,dirtyFiles:actual,attestedAt:clock().toISOString()};
