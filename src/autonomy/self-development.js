...
export function createSelfDevelopmentService({
  ...
  async function recoverImplementationPlan(taskId, input) {
    ...
    const priorPartialEvidenceRecovery=(current.metadata?.implementationPlanRecoveryHistory||[]).find(item=>item.recoveryClass==="partial_repair_plan_evidence_rebind"&&item.previousStateVersion===input.expectedVersion);
    ...
    const partialSteps=await runtime.steps(current.id),latestFailed=partialSteps.filter(step=>step.status==="failed").at(-1),partialRecord=(current.metadata?.partialRepairPlanRecoveryHistory||[]).at(-1),partialFailure=latestFailed?.stepId===`${current.currentStep+1}:plan_repair`&&latestFailed.stepType==="plan_repair"&&latestFailed.errorCode==="implementation_evidence_incomplete"&&latestFailed.input?.arguments?.failureEvidence?.code==="repair_plan_incomplete"&&latestFailed.result?.message==="Task-owned repair evidence no longer matches its durable lineage.",failureEvidence=latestFailed?.input?.arguments?.failureEvidence,requiredPaths=partialRecord?.requiredPaths,partialEntries=partialRecord?.entries,partialExact=current.status==="failed"&&current.errorCode==="implementation_evidence_incomplete"&&partialFailure&&current.branch===approvedBranch&&current.metadata?.selfDevelopment?.repository===repository&&/^[0-9a-f]{40}$/i.test(runtimeVersion||"")&&partialRecord?.taskId===current.id&&partialRecord.repository===repository&&partialRecord.branch===approvedBranch&&partialRecord.currentCommit===current.currentCommit&&Array.isArray(requiredPaths)&&requiredPaths.length>0&&requiredPaths.length<=12&&new Set(requiredPaths).size===requiredPaths.length&&Array.isArray(partialEntries)&&partialEntries.length===requiredPaths.length&&partialEntries.every(entry=>requiredPaths.includes(entry.path)&&/^[0-9a-f]{64}$/i.test(entry.contentHash||""))&&partialRecord.fingerprint===failureEvidence?.fingerprint&&partialRecord.sourcePlanStepId===failureEvidence?.sourcePlanStepId&&partialRecord.sourceApplyStepId===failureEvidence?.sourceApplyStepId&&JSON.stringify(requiredPaths)===JSON.stringify(failureEvidence?.requiredPaths)&&partialRecord.activeContinuation?.generationId===current.metadata?.activeContinuation?.generationId&&!current.leaseOwner&&!current.leaseToken&&!current.approvalState&&!partialSteps.some(step=>Number.parseInt(step.stepId,10)>Number.parseInt(latestFailed.stepId,10));
    if(partialExact){
      ...
      const nextSteps=remaining.map((step,index)=>({...step,idempotencyIdentity:`${step.idempotencyIdentity||`self-development:${step.type}`}:partial-evidence-rebind:${base+index+1}`})),now=clock().toISOString(),activeContinuation=createActiveContinuation({task:current,startStep:base,plannedSteps:nextSteps.length,repairLimit:current.metadata?.maxRepairIterations??2,recoveryClass:"partial_repair_plan_evidence_rebind",runtimeStartedAt:now,runtimeMinutes:15}),record={recoveryClass:"partial_repair_plan_evidence_rebind",previousStateVersion:current.stateVersion,failedStepId:latestFailed.stepId,sourcePlanStepId:partialRecord.sourcePlanStepId,sourceApplyStepId:partialRecord.sourceApplyStepId,fingerprint:partialRecord.fingerprint,runtimeVersion:runtimeVersion||null,recoveredAt:now};
      const updated=await storage.updateAutonomyTask(current.id,ownerId,{status:"queued",currentStep:base,currentPhase:"plan_repair",nextRunAt:now,startedAt:now,completedAt:null,errorCode:null,retryCount:0,blockedReason:null,checkpoint:{...current.checkpoint,pendingStep:null},metadata:{...current.metadata,steps:[...current.metadata.steps,...nextSteps],requiredCapability:"reasoning",autoDispatch:true,activeContinuation,continuationHistory:[...(current.metadata?.continuationHistory||[]),activeContinuation],implementationPlanRecoveryHistory:[...(current.metadata?.implementationPlanRecoveryHistory||[]),record]}},current.stateVersion);
