...
export function createSelfDevelopmentService({
  ...
  async function recoverHandsCommitMismatch(taskId,input,{failureCode="commit_mismatch",recoveryClass="commit_mismatch_descendant_rebind"}={}){
    ...
    const steps=await runtime.steps(current.id),failed=steps.find(step=>step.stepId===`${current.currentStep+1}:apply_patch`&&step.status==="failed"&&step.errorCode===failureCode),planned=steps.find(step=>step.stepId===`${current.currentStep}:plan_implementation`&&step.status==="completed"&&step.result?.implementationPlan),approvals=await storage.listApprovals(ownerId,{limit:100}),remaining=current.metadata?.steps?.slice(current.currentStep)||[],completedAfterPlan=steps.some(step=>Number.parseInt(step.stepId,10)>current.currentStep&&["apply_patch","run_focused_tests","run_full_tests","commit","review_commit","push","deploy_preview"].includes(step.stepType)&&step.status==="completed"),oldCommit=current.currentCommit,newCommit=currentCommit;
    ...
    const evidence=await compareRemoteEvidence({repository,paths:evidencePaths,oldCommit,newCommit}),changedPaths=evidencePaths.filter(path=>evidence[path]?.equivalent!==true),base=current.metadata.steps.length,planTemplate=current.metadata.steps[current.currentStep-1];if(!planTemplate||planTemplate.type!=="plan_implementation")throw new SelfDevelopmentError("hands_context_recovery_precondition_failed","The exact implementation plan boundary is unavailable.");
    ...
    const now=clock().toISOString(),record={recoveryClass,previousStateVersion:current.stateVersion,failedStepId:failed.stepId,previousCurrentCommit:oldCommit,newCurrentCommit:newCommit,ancestryVerified:true,evidence:Object.fromEntries(evidencePaths.map(path=>[path,{oldBlob:evidence[path].oldSha,newBlob:evidence[path].newSha,equivalent:evidence[path].equivalent===true}])),invalidatedEvidencePaths:changedPaths,repositoryRoot:input.workspace.root,gitTopLevel:input.workspace.gitTopLevel,workingTreeClean:true,recoveredAt:now},requiredCapability=changedPaths.length?"repo_read_remote":"repo_mutate_local",status=changedPaths.length?"queued":"waiting_for_worker";
    const updated=await storage.updateAutonomyTask(current.id,ownerId,{status,currentStep:base,currentPhase:"hands_repository_context_recovery",currentCommit:newCommit,maxSteps,nextRunAt:now,startedAt:now,completedAt:null,errorCode:null,retryCount:0,blockedReason:null,checkpoint:{...current.checkpoint,pendingStep:null},metadata:{...current.metadata,steps:[...current.metadata.steps,...nextSteps],requiredCapability,autoDispatch:true,selfDevelopmentImplementationPlan:changedPaths.length?null:current.metadata.selfDevelopmentImplementationPlan,handsContextRecoveryHistory:[...(current.metadata.handsContextRecoveryHistory||[]),record],baseRevisionHistory:[...(current.metadata.baseRevisionHistory||[]),{previousBaseCommit:oldCommit,newBaseCommit:newCommit,previousStateVersion:current.stateVersion,ancestryVerified:true,recoveredAt:now}] }},current.stateVersion);if(!updated)throw new SelfDevelopmentError("version_conflict","Task changed during Hands-context recovery.");
